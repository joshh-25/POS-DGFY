...
export const platformInvoiceRepository = {
  ...
  findOriginalForApplication(registrationApplicationId, transaction) { return PlatformInvoice.findOne({ where: { registration_application_id: registrationApplicationId, invoice_kind: 'original', invoice_status: { [Op.ne]: 'discarded' } }, transaction }); },
  ...
  discardDraft(invoice, transaction) { return invoice.update({ invoice_status: 'discarded', original_registration_application_id: null }, { transaction }); },
  ...
  findLatestDelivery(invoiceId, transaction, { lock = false } = {}) { return PlatformInvoiceDelivery.findOne({ where: { invoice_id: invoiceId }, order: [['created_at', 'DESC']], transaction, lock: lock ? transaction?.LOCK?.UPDATE : undefined }); },
