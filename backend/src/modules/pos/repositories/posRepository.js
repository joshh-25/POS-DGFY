...
export const posRepository = {
    async getReportsOverview(filters = {}, options = {}) {
        ...
        const dateFrom = normalizeBusinessDateValue(filters.date_from) || normalizeBusinessDateValue(new Date());
        const dateTo = normalizeBusinessDateValue(filters.date_to) || dateFrom;
        const where = buildFinanciallyRecognizedSalesWhere({
            created_at: { [Op.between]: [toDateStart(dateFrom), toDateEnd(dateTo)] }
        });
        ...
        if (filters.payment_type) where.payment_type = String(filters.payment_type).trim();
        if (filters.terminal_id) where.terminal_id = String(filters.terminal_id).trim();
        if (filters.source) {
            const source = String(filters.source).trim();
            if (['delivery', 'pickup'].includes(source)) {
                where[Op.and] = [...(where[Op.and] || []), { order_method: source }];
            } else {
        ...
        const transactions = rows.map(toPlain).map((transaction) => {
            if (!filters.category) return transaction;
            return {
                ...transaction,
                lines: (transaction.lines || []).filter((line) => (
                    String(line?.item?.category || '') === String(filters.category)
                ))
            };
        }).filter((transaction) => !filters.category || transaction.lines.length > 0);
        const money = (value) => round4(value);
        const emptySummary = () => ({
            gross_sales: 0, net_sales: 0, discounts: 0, refunds_voids: 0, vat: 0,
            service_fees: 0, cogs: 0, pos_profit_loss: 0, profit_margin: 0,
            total_transactions: 0, is_loss: false
        });
        const summarize = (items) => {
            const summary = emptySummary();
            items.forEach((transaction) => {
                const refunded = ['refunded', 'partial_refunded'].includes(transaction.payment_status)
                    || ['voided', 'cancelled'].includes(transaction.status);
                const gross = money(transaction.subtotal_amount);
                const discount = money(transaction.discount_amount);
                const net = money(gross - discount);
                const cost = money((transaction.lines || []).reduce((sum, line) => (
                    sum + (Number(line.quantity || 0) * Number(line.cost_snapshot ?? line.item?.cost_per_unit ?? 0))
                ), 0));
                summary.gross_sales += gross;
                summary.discounts += discount;
                summary.refunds_voids += refunded ? net : 0;
                summary.net_sales += refunded ? 0 : net;
                summary.vat += money(transaction.vat_amount);
                summary.service_fees += money(transaction.service_fee_amount) + money(transaction.restaurant_service_charge_amount);
                summary.cogs += cost;
                summary.total_transactions += 1;
            });
            Object.keys(summary).forEach((key) => {
                if (typeof summary[key] === 'number') summary[key] = money(summary[key]);
            });
            summary.pos_profit_loss = money(summary.net_sales - summary.cogs);
            summary.profit_margin = summary.net_sales > 0 ? money((summary.pos_profit_loss / summary.net_sales) * 100) : 0;
            summary.is_loss = summary.pos_profit_loss < 0;
            return summary;
        };
        const groupRows = (keyResolver) => {
            const groups = new Map();
            transactions.forEach((transaction) => {
                const key = keyResolver(transaction);
                if (!groups.has(key)) groups.set(key, []);
                groups.get(key).push(transaction);
            });
            return groups;
        };
        const breakdown = (keyResolver, keyName) => Array.from(groupRows(keyResolver)).map(([key, items]) => ({
            [keyName]: key || 'Unknown',
            ...summarize(items)
        }));
        ...
        transactions.forEach((transaction) => {
            const amount = money(transaction.discount_amount);
            if (amount <= 0) return;
            const governed = transaction.discount;
            const type = String(governed?.discount_type || '').trim();
            const label = String(transaction.discount_label_snapshot || '').trim();
            const key = type || label || 'manual';
            const current = discountGroups.get(key) || {
                discount_type: type || 'legacy',
                discount_label: label || (type ? type.replace(/_/g, ' ') : 'Manual discount'),
                transaction_count: 0,
                discount_amount: 0,
                vat_removed: 0
            };
            current.transaction_count += 1;
            current.discount_amount = money(current.discount_amount + amount);
            current.vat_removed = money(current.vat_removed + money(governed?.vat_removed));
            discountGroups.set(key, current);
        });
        const cashierGroups = groupRows((transaction) => transaction.cashier_id || 'unassigned');
        const cashierSummary = Array.from(cashierGroups).map(([id, items]) => {
            const shifts = new Map();
            items.forEach((transaction) => {
                const shift = transaction.shift;
                if (shift?.pos_terminal_shift_id) {
                    shifts.set(shift.pos_terminal_shift_id, {
                        ...(shifts.get(shift.pos_terminal_shift_id) || {}),
                        ...shift
                    });
                }
            });
            const closedShifts = Array.from(shifts.values()).filter((shift) => shift.status === 'closed');
            return {
                cashier_id: toPositiveInt(id),
                cashier_name: items[0]?.cashier?.username || items[0]?.cashier?.email || 'Unassigned',
                shift_ids: Array.from(shifts.keys()),
                summary: summarize(items),
                shift_money: {
                    opening_float_amount: money(closedShifts.reduce((sum, shift) => sum + Number(shift.opening_float_amount || 0), 0)),
                    expected_cash_amount: money(closedShifts.reduce((sum, shift) => sum + Number(shift.expected_cash_amount || 0), 0)),
                    closing_cash_amount: money(closedShifts.reduce((sum, shift) => sum + Number(shift.closing_cash_amount || 0), 0)),
                    cash_variance_amount: money(closedShifts.reduce((sum, shift) => sum + Number(shift.cash_variance_amount || 0), 0)),
                    closed_shift_count: closedShifts.length
                }
            };
        });
        const shiftSummary = breakdown((transaction) => transaction.shift_id || 'unassigned', 'shift_id').map((entry) => {
            const transaction = transactions.find((item) => String(item.shift_id || 'unassigned') === String(entry.shift_id));
            return {
                ...entry,
                terminal_id: transaction?.shift?.terminal_id || transaction?.terminal_id || null,
                business_date: transaction?.shift?.business_date || null,
                status: transaction?.shift?.status || null,
                opening_float_amount: money(transaction?.shift?.opening_float_amount),
                expected_cash_amount: money(transaction?.shift?.expected_cash_amount),
                closing_cash_amount: money(transaction?.shift?.closing_cash_amount),
                cash_variance_amount: money(transaction?.shift?.cash_variance_amount),
                summary: {
                    gross_sales: entry.gross_sales,
                    net_sales: entry.net_sales,
                    pos_profit_loss: entry.pos_profit_loss,
                    total_transactions: entry.total_transactions
                }
            };
        });
        const itemGroups = new Map();
        transactions.forEach((transaction) => (transaction.lines || []).forEach((line) => {
            const item = line.item || {};
            const key = line.item_id || item.item_id;
            const current = itemGroups.get(key) || { item_id: key, item_name: item.name || `Item #${key}`, sku_code: item.sku_code || null, category: item.category || null, quantity: 0, net_sales: 0, cogs: 0 };
            const lineSubtotal = (transaction.lines || []).reduce((sum, entry) => sum + Number(entry.line_subtotal || 0), 0);
            const transactionNet = Math.max(0, Number(transaction.subtotal_amount || 0) - Number(transaction.discount_amount || 0));
            const discountFactor = lineSubtotal > 0
                ? transactionNet / lineSubtotal
                : 1;
            current.quantity += Number(line.quantity || 0);
            current.net_sales += Number(line.line_subtotal || 0) * discountFactor;
            current.cogs += Number(line.quantity || 0) * Number(line.cost_snapshot ?? item.cost_per_unit ?? 0);
            itemGroups.set(key, current);
        }));
        const topItems = Array.from(itemGroups.values()).map((item) => ({
            ...item,
            quantity: money(item.quantity), net_sales: money(item.net_sales), cogs: money(item.cogs),
            pos_profit_loss: money(item.net_sales - item.cogs)
        })).sort((a, b) => b.net_sales - a.net_sales).slice(0, 10);
        const trend = breakdown((transaction) => normalizeBusinessDateValue(transaction.created_at), 'period').map((entry) => ({
            period: entry.period, gross_sales: entry.gross_sales, net_sales: entry.net_sales, pos_profit_loss: entry.pos_profit_loss
        }));
        const paymentBreakdown = breakdown((transaction) => transaction.payment_type, 'payment_method');
        const orderMethodBreakdown = breakdown((transaction) => transaction.order_method, 'order_method');
        const filterCashiers = Array.from(cashierGroups).map(([id, items]) => ({ cashier_id: toPositiveInt(id), cashier_name: items[0]?.cashier?.username || items[0]?.cashier?.email || 'Unassigned' }));
        const categories = [...new Set(transactions.flatMap((transaction) => (transaction.lines || []).map((line) => line?.item?.category)).filter(Boolean))];
        const commonReport = { summary, trend, payment_breakdown: paymentBreakdown, order_method_breakdown: orderMethodBreakdown, top_items: topItems };
        ...
        return {
            ...
            filter_options: { cashiers: filterCashiers, categories },
            daily_report: { ...commonReport, discount_breakdown: Array.from(discountGroups.values()), cashier_summary: cashierSummary, shift_summary: shiftSummary },
            monthly_report: { summary, sales_trend: trend, top_items: topItems },
            yearly_report: { summary, monthly_breakdown: trend, top_items: topItems },
            sales_comparison: { fixed_periods: [], monthly_trend: trend },
            profit_loss: summary
        };
    },
