...
async function restoreAdmin() {
    ...
    try {
        ...
        const allPermissions = [
            ...
            "jo:view", "jo:create", "jo:edit", "jo:approve", "jo:complete", "jo:delete",
        ...
        if (users.length > 0) {
            ...
            await sequelize.query(
                `UPDATE Users SET password_hash = ?, role = 'admin', is_active = 1, permissions = ? WHERE email = ?`,
                { replacements: [passwordHash, permissionsJson, email] }
            );
        } else {
            console.log('User does not exist. Creating...');
            await sequelize.query(
                `INSERT INTO Users (username, email, password_hash, role, is_active, permissions, created_at, updated_at) 
                 VALUES ('BillingTestAdmin', ?, ?, 'admin', 1, ?, NOW(), NOW())`,
                { replacements: [email, passwordHash, permissionsJson] }
            );
        }
