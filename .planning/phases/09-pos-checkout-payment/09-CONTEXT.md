...
### Resolved During Planning Research (User-Confirmed 2026-07-13)

*These four decisions resolve assumptions A1–A4 flagged by 09-RESEARCH.md. They override any ambiguity in D-05/D-07/D-13/D-15.*

- **D-19:** **Prices are VAT-INCLUSIVE.** Stored Product prices already include the 12% VAT (Philippine retail convention). VAT is decomposed server-side per line from the line total `L`: `vat = round(L − L/1.12)`, `net = L − vat`. Receipt shows the VAT-inclusive line total, with the VAT component broken out. This resolves research assumption A1.

- **D-20:** **SC/PWD sales are VAT-EXEMPT.** For an Availment with the SC/PWD statutory discount applied, the 12% VAT is zeroed entirely AND the 20% discount is applied to the VAT-exclusive (net) base — the BIR-correct treatment under RA 9994 / RA 10754. The SC/PWD receipt shows the VAT-exempt sale and the 20%-off line separately. This resolves research assumption A2 and refines D-07 (D-07's "20% off VAT-exclusive base" is retained AND the VAT is exempted, not merely excluded from the discount base).

- **D-21:** **Phase 9 supports BOTH fiscal and non_fiscal checkout contexts.** Finalization must handle `compliant_active` businesses issuing fiscal receipts, which per Phase 8's FSC-02 gap-closure requires passing the compliance gate's full evidence checklist (the 7-signal `assertComplianceGate` evidence set), in addition to the simple `non_compliant_active` + `non_fiscal` ALLOW path. This is a deliberate scope expansion beyond research assumption A4 — the finalize usecase must gather and supply the compliance-evidence inputs the fiscal gate requires. `requestedDocumentContext` (fiscal | non_fiscal) is still passed per the client app build (D-15).

- **D-22:** **Receipt-print failure = record-then-warn (fail-open).** At finalization, persist the Availment / Payment / Receipt to the DB first, then attempt synchronous device-bridge printing best-effort. If device-bridge is unavailable or the print fails, the transaction still completes (money is never lost to a printer fault); staff receive a warning and a reprint path. This resolves research assumption A3 and the D-11 Claude's-discretion timeout/error item. (Note: this refines D-11's "blocks finalization until receipt completes" — the DB write commits first; only the physical print is best-effort.)

- **D-23:** **Fiscal evidence via an interim per-business attestation store.** Rather than build seven real compliance-evidence producer subsystems (fiscal accumulator, append-only audit, encryption, submission docs, RMO filing, fiscal-terminal registration, payment-handoff), Phase 9 adds a per-business `compliance_evidence` attestation table. An operator attests the required signals/settings/artifacts/peripherals once; the finalize usecase reads that store and assembles the evidence bundle passed to `assertComplianceGate` for `compliant_active`/fiscal checkout. This gets real fiscal receipts working in Phase 9 without building the full compliance subsystems (those remain a future phase). Resolves research assumption A8 (evidence-store shape = interim attestation, "option B"). NOTE (research A9): `compliant_active` businesses run the evidence checklist even for `non_fiscal` checkout, so the attestation store is consulted for any `compliant_active` finalize, not only fiscal ones.

- **D-24:** **Fiscal checkout hard-fails on incomplete evidence.** When a fiscal (`compliant_active`) checkout cannot satisfy the evidence bundle — the gate throws REQUIRES_SETUP (409) or BLOCKED/DENY (403) — the finalize usecase REJECTS the sale and surfaces exactly which signals are missing (from the gate error's `checklist.missing_*` / `activation_blockers[]`). Phase 9 does NOT silently downgrade a fiscal sale to a non_fiscal slip. Staff/operator must complete the attestation before fiscal sales proceed. Resolves research assumption A7 (hard-fail branch). BIR-safe: a fiscal receipt is never issued without complete evidence.
