...
- **GSD validation scope**: Do not run root `npm test` for apps-scoped migration/API phases. The root test command pulls both `frontend/` and `backend/`, which is outside the intended write/validation scope for current `apps/*` work. Validate only the touched app surface instead, such as `cd apps/dgfy-migration-runner && npm test` or the relevant `apps/dgfy-api` targeted suite, unless the user explicitly asks for root/frontend/backend-wide testing.
