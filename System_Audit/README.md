...
3. Treat the latest production SHA evidenced in this workspace as `52c8dfc8f232aa5e3e926ea75f2baf96d389d425`. The evidence is based on production deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260615_153953.summary.txt`, backend/IMS/POS/Store health, public endpoint checks, asset parity, tenant schema sync, tenant schema sync regression, permission backfill, Storefront discovery reconciliation, and local/origin/remote/deploy-marker parity. Tenant index headroom ran non-strict because redundant-index warnings remain a tracked cleanup concern. The latest controlled DGFY production mutation UAT remains the June 13 run, which proved OTP delivery, fresh DGFY account registration, automatic company activation, IMS tenant-session handoff, onboarding item creation, DGFY-authenticated Storefront checkout, POS completion, inventory decrement from `10` to `9`, and DGFY account order visibility with ratings at or above `9.1`. The June 15 release still used the auditable emergency no-staging bypass for stale QA deployed-head evidence, so `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` remains open until the QA parity proof is replaced or made deterministic.
4. Treat Storefront public visibility opt-in as production-deployed only for the runtime code included in SHA `023e5f9ff0c71340cb9f92a9a7d7526920a62e05`; production tenant-data cleanup still requires a production `audit:storefront-public-visibility` run and hidden/visible tenant discovery/profile smoke before claiming production data closure.
5. Reduce production performance risk: StorefrontApp decomposition, high-limit query review, upload transport validation.
6. Close governance debt: RBAC fallback removal, architecture allowlist removal plan, fiscal/payment evidence.
7. Add operational proof: SLO dashboards, alert evidence, AI adversarial/cost tests, release evidence that no longer depends on stale QA bypasses.
