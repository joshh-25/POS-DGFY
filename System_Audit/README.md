...
- Production deployment on 2026-06-09 deployed code target SHA `4a7b924af57aa8709b673c75baf673625f0cc170` using the documented emergency no-staging bypass because stale QA deployed-head evidence was the sole failed gate after QA smoke, rollback/restore, docs, and architecture checks passed. Deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260609_230643.summary.txt` reported backend health, IMS/POS/Store runtime checks, public endpoint checks, tenant-store asset integrity, frontend asset parity, tenant schema sync, tenant schema sync regression, tenant index headroom, permission backfill, Storefront discovery index reconciliation, and PM2 reload passing. PayMongo/payment changes were intentionally excluded from the deployed commit. Remote `HEAD`, remote `origin/master`, and `.deploy-state/last_deployed_commit` matched the deployed SHA at proof time.
- Current repository HEAD is `5e327ba74fd83aa457339233536232efdd8fa8d0` as of the 2026-06-10 documentation refresh and matches `origin/master`. The commit after the latest production evidence includes storefront gallery preservation work; it requires fresh deploy evidence before the audit should treat it as production runtime.
...
3. Treat the latest production code SHA evidenced in this workspace as `4a7b924af57aa8709b673c75baf673625f0cc170` based on the production deploy summary, health, asset parity, tenant schema sync, tenant index headroom, permission backfill, and Storefront discovery reconciliation evidence from `/var/www/skupervisor/logs/deploy/deploy_20260609_230643.summary.txt`. The release used the auditable emergency no-staging bypass for stale QA deployed-head evidence, so `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` remains open until the QA parity proof is replaced or made deterministic. Do not advance production claims to current repository HEAD `5e327ba74fd83aa457339233536232efdd8fa8d0` without a fresh deploy summary.
