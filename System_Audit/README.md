...
- Production deployment on 2026-06-09 deployed code target SHA `9cab0803a9bdf7e0d7238f94c9db255d9df30146` using the documented emergency no-staging bypass because stale QA deployed-head evidence was the sole failed gate after QA smoke, rollback/restore, docs, and architecture checks passed. Deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260609_180115.summary.txt` reported backend health, IMS/POS/Store runtime checks, public endpoint checks, tenant-store asset integrity, frontend asset parity, tenant schema sync, tenant schema sync regression, tenant index headroom, permission backfill, Storefront discovery index reconciliation, and PM2 reload passing. PayMongo/payment changes were intentionally excluded from the deployed commit.
...
3. Treat the current production code SHA as `9cab0803a9bdf7e0d7238f94c9db255d9df30146` based on the production deploy summary, health, asset parity, tenant schema sync, tenant index headroom, permission backfill, and Storefront discovery reconciliation evidence from `/var/www/skupervisor/logs/deploy/deploy_20260609_180115.summary.txt`. The release used the auditable emergency no-staging bypass for stale QA deployed-head evidence, so `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` remains open until the QA parity proof is replaced or made deterministic.
