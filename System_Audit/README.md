...
- Production deployment on 2026-06-10 deployed runtime code target SHA `9e7c64ceaaa6e0f636db0cf59e413874b03f7b70` using the documented emergency no-staging bypass because stale QA deployed-head evidence was the sole failed gate after QA smoke, rollback/restore, docs, and architecture checks passed. Deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260610_151425.summary.txt` reported backend health, IMS/POS/Store runtime checks, public endpoint checks, tenant-store asset integrity, frontend asset parity, tenant schema sync, tenant schema sync regression, tenant index headroom, permission backfill, Storefront discovery index reconciliation, and PM2 reload passing. Remote `HEAD`, remote `origin/master`, and `.deploy-state/last_deployed_commit` matched the deployed SHA at proof time.
- DGFY signup OTP scope hotfix code commit `4be34b7a` keeps `dgfy_account_verification` OTP requests global even when a browser still carries tenant context, matching the global `/api/v1/dgfy/auth/register` verifier. Promote a SHA containing this commit before treating the browser OTP fix as production runtime.
...
3. Treat the latest production runtime code SHA evidenced in this workspace as `9e7c64ceaaa6e0f636db0cf59e413874b03f7b70` until a SHA containing hotfix code commit `4be34b7a` is promoted with a fresh deploy summary. The `9e7c64ce...` evidence is based on production deploy summary, health, asset parity, tenant schema sync, tenant index headroom, permission backfill, Storefront discovery reconciliation evidence from `/var/www/skupervisor/logs/deploy/deploy_20260610_151425.summary.txt`, and live route smoke for `/dgfy/auth` plus `/register-company`. The release used the auditable emergency no-staging bypass for stale QA deployed-head evidence, so `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` remains open until the QA parity proof is replaced or made deterministic.
