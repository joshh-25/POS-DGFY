...
- The latest production runtime evidence available in this workspace is June 16, 2026 target SHA `039f048033577cad1de7470e79a162eabc3e5e75`. Deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260616_163311.summary.txt` records backend/IMS/POS/Store checks, public endpoint checks, tenant-store asset integrity, frontend asset parity, tenant schema sync/regression, permission backfill, Storefront discovery reconciliation, PM2 reload passing, and `tenant_index_headroom_strict=0` report mode. Production remote `HEAD` and `.deploy-state/last_deployed_commit` matched this SHA at proof time; later documentation-only commits may advance local or `origin/master` without changing runtime. This deploy includes DGFY account company switching, pending IMS email invitation visibility from the DGFY Business area, email-only business step-up reuse, no `company_token` exposure in the new switcher/invitation payloads, Storefront selected-company switching, the no-location Storefront hardening chain already in the deployed commit history, and the deploy-wrapper fix that forwards `DEPLOY_TENANT_INDEX_HEADROOM_STRICT` to the production-side script. Landlord migration `20260616000001-add-dgfy-company-switching.cjs` is `up` in production.
...
3. Treat the latest production runtime SHA evidenced in this workspace as `039f048033577cad1de7470e79a162eabc3e5e75`. The evidence is based on production deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260616_163311.summary.txt`, backend/IMS/POS/Store health, public endpoint checks, asset parity, tenant schema sync, tenant schema sync regression, permission backfill, Storefront discovery reconciliation, PM2 reload, route-level DGFY `401` smoke, DGFY company-switching migration status, and production remote/deploy-marker parity. Later documentation-only commits may advance local or `origin/master` without changing runtime. Tenant index headroom ran non-strict report mode because redundant-index warnings remain a tracked cleanup concern. Space Bar Storefront access proof remains requested `transaction` / effective `catalog`, with checkout disabled because registration stage `informal` allows up to catalog mode. The latest controlled DGFY production mutation UAT remains the June 13 run, which proved OTP delivery, fresh DGFY account registration, automatic company activation, IMS tenant-session handoff, onboarding item creation, DGFY-authenticated Storefront checkout, POS completion, inventory decrement from `10` to `9`, and DGFY account order visibility with ratings at or above `9.1`. The June 16 release still used the auditable emergency no-staging bypass for stale QA deployed-head evidence, so `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` remains open until the QA parity proof is replaced or made deterministic.
