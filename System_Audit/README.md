...
- The latest production deployment evidence available in this workspace is June 16, 2026 target SHA `14de6e0f0d4497d7821e047704a66705b6164f29`. Deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260616_021931.summary.txt` records backend/IMS/POS/Store checks, public endpoint checks, tenant-store asset integrity, frontend asset parity, tenant schema sync/regression, permission backfill, Storefront discovery reconciliation, PM2 reload passing, and `tenant_index_headroom_strict=0` report mode. Local `master`, `origin/master`, production remote `HEAD`, and `.deploy-state/last_deployed_commit` matched this SHA at proof time. This deploy includes the Storefront access-mode ceiling UI correction: Tenant Manager owns the platform max allowed selector, tenant Settings can request up to that platform ceiling, and backend effective-mode enforcement still blocks quote, checkout, booking, and payment until effective mode is `transaction` and all existing gates pass.
