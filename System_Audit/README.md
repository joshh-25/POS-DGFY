...
- The latest production runtime evidence available in this workspace is June 17, 2026 target SHA `156b7a199e359efe8731e4afbdafccf0b06c8b4a`. Deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260617_020652.summary.txt` records backend/IMS/POS/Store checks, public endpoint checks, tenant-store asset integrity, frontend asset parity, tenant schema sync/regression, permission backfill, Storefront discovery reconciliation, PM2 reload passing, and `migrations_changed=0`. Production remote `HEAD` and `.deploy-state/last_deployed_commit` matched this SHA at proof time; later documentation-only commits may advance local or `origin/master` without changing runtime. This deploy includes DGFY account company switching, pending IMS email invitation visibility from the DGFY Business area, email-only business step-up reuse, no `company_token` exposure in the new switcher/invitation payloads, Storefront selected-company switching, the no-location Storefront hardening chain, Storefront hours persistence, company-switcher current-company rendering, F&B `Menu Item` onboarding, shared MapLibre address autofill, and selected-image carousel append/remove behavior.
...
- Source-current DGFY company-access hardening is implemented locally and ready for guarded staging proof. It tightens ownership transfer to `tenants.owner_dgfy_account_id`, demotes the former owner after transfer so leave-company rules are accurate, scopes DGFY account service failures away from generic tenant global toasts, makes development auto-login opt-in and excludes DGFY/POS lock entrypoints, and expands the DGFY rendered-smoke script with strict-network mode, localhost HTTPS-forwarding, seeded text checks, and structured JSON evidence under `.tmp/rendered-qa/dgfy-access/`. This source evidence does not advance production runtime beyond SHA `156b7a199e359efe8731e4afbdafccf0b06c8b4a`; seeded POS hardware UAT and strict local/staging DGFY access smoke remain required before production readiness can exceed the control matrix residual-risk rating.
...
3. Treat the latest production runtime SHA evidenced in this workspace as `156b7a199e359efe8731e4afbdafccf0b06c8b4a`. The evidence is based on production deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260617_020652.summary.txt`, backend/IMS/POS/Store health, public endpoint checks, asset parity, tenant schema sync, tenant schema sync regression, permission backfill, Storefront discovery reconciliation, PM2 reload, `migrations_changed=0`, focused frontend map/onboarding/image/settings tests, SKUpervisor build, and production remote/deploy-marker parity. Later documentation-only commits may advance local or `origin/master` without changing runtime. Space Bar Storefront access proof remains requested `transaction` / effective `catalog`, with checkout disabled because registration stage `informal` allows up to catalog mode. The latest controlled DGFY production mutation UAT remains the June 13 run, which proved OTP delivery, fresh DGFY account registration, automatic company activation, IMS tenant-session handoff, onboarding item creation, DGFY-authenticated Storefront checkout, POS completion, inventory decrement from `10` to `9`, and DGFY account order visibility with ratings at or above `9.1`. The June 17 release still used the auditable emergency no-staging bypass for stale QA deployed-head evidence, so `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` remains open until the QA parity proof is replaced or made deterministic.
