...
- The latest production runtime evidence available in this workspace is June 19, 2026 proof for target SHA `207aa434b4627020cbd286f5e6b5baaf8cebe57a`. Production summary `/var/www/skupervisor/logs/deploy/deploy_20260619_105411.summary.txt` records `deployed_head`, `remote_head`, and `expected_commit` all matching the target SHA, frontend asset parity `pass`, tenant schema sync report output, tenant index headroom report output, `tenant_index_headroom_strict=0`, `migrations_changed=0`, and `total_changed_files=0`. Live `/api/v1/health` reports `services.observability.runtime_sha=207aa434b4627020cbd286f5e6b5baaf8cebe57a` with source `deploy_state:last_deployed_commit`; database, Redis, runtime schema, schema indexes, billing telemetry, and observability are healthy, with tenant pool capacity warning at 20 of 20. The no-staging verdict for this target used the documented emergency bypass because stale QA deploy-summary SHA parity was the only failed pre-deploy gate; post-deploy runtime SHA, deploy summary, and frontend asset parity match the target. The current runtime chain includes DGFY company access hardening, PR #20 POS/Storefront customer-flow work, post-merge DGFY security hardening, PR #18 Storefront discovery ranking, F&B mobile hero stabilization, restored Storefront customer dashboard layout, June 18 CSRF/POS terminal-lock remediation, POS `Lock Terminal` open-shift choice, iMin receipt preview scrolling, Storefront auth/order tracking hardening, Storefront/POS browser-authenticated request consolidation, and DGFY account in-app notifications/SSE. Protected notification endpoints were read-only production checked for clean unauthenticated `401`; live production order-mutation UAT is still needed for notification creation and status propagation proof.
...
- `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` - remediated 2026-06-18 with no-staging SHA parity passing without emergency bypass, live health runtime SHA proof, and observability report-mode evidence for target SHA `4cf4c372c9eae91c6ead5932e19476f4c1b98e83`; the later `27e5ce12` deploy used the documented stale-QA emergency bypass before deploy, but post-deploy runtime SHA, deploy summary, and frontend asset parity match the target.
...
3. Treat the latest production runtime SHA evidenced in this workspace as `207aa434b4627020cbd286f5e6b5baaf8cebe57a`. The evidence is based on production deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260619_105411.summary.txt`, production remote `HEAD`, `.deploy-state/last_deployed_commit`, live `/api/v1/health.services.observability.runtime_sha`, frontend asset parity, tenant schema sync report output, tenant index headroom report output, `tenant_index_headroom_strict=0`, `migrations_changed=0`, and `total_changed_files=0`. The no-staging verdict for this target used the documented emergency bypass because stale QA deploy-summary SHA parity was the only failed pre-deploy gate; do not treat it as a clean no-bypass proof, but do treat the post-deploy runtime parity as current production evidence. The latest controlled DGFY production mutation UAT remains the June 13 run, which proved OTP delivery, fresh DGFY account registration, automatic company activation, IMS tenant-session handoff, onboarding item creation, DGFY-authenticated Storefront checkout, POS completion, inventory decrement from `10` to `9`, and DGFY account order visibility with ratings at or above `9.1`. Authenticated cashier/admin UAT and controlled live notification/status-propagation UAT remain the main human signoff gaps.
4. Treat the Platform Admin compliance lifecycle switch as source-current until a guarded production deploy records a new SHA and smoke evidence. The switch can move tenants into `non_compliant_active` or `compliant_pending` only; it cannot bypass checklist activation into `compliant_active`.
...
9. Promote the repo-native observability v1 slice with guarded production evidence, then close remaining operational proof: SLO dashboards, alert evidence, AI adversarial/cost tests, release evidence that no longer depends on stale QA bypasses.
