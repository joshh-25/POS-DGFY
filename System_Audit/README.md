...
- At the start of this documentation refresh, local `master`, `origin/master`, and production runtime evidence matched `9e7c64ceaaa6e0f636db0cf59e413874b03f7b70`. Documentation-only commits after this refresh do not change production runtime until separately pushed and promoted.
...
3. Treat the latest production runtime code SHA evidenced in this workspace as `9e7c64ceaaa6e0f636db0cf59e413874b03f7b70` based on the production deploy summary, health, asset parity, tenant schema sync, tenant index headroom, permission backfill, Storefront discovery reconciliation evidence from `/var/www/skupervisor/logs/deploy/deploy_20260610_151425.summary.txt`, and live route smoke for `/dgfy/auth` plus `/register-company`. The release used the auditable emergency no-staging bypass for stale QA deployed-head evidence, so `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` remains open until the QA parity proof is replaced or made deterministic. Do not advance production runtime claims beyond this SHA without a fresh deploy summary.
