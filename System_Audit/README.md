...
3. Treat the latest production runtime SHA evidenced in this workspace as `27e5ce1249136ef8676392fec5b6e2c027d459e5`. The evidence is based on production deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260618_194442.summary.txt`, production remote `HEAD`, `.deploy-state/last_deployed_commit`, live `/api/v1/health.services.observability.runtime_sha`, frontend asset parity, tenant schema sync report output, tenant index headroom report output, `tenant_index_headroom_strict=0`, `migrations_changed=0`, and `total_changed_files=0`. The no-staging verdict for this target used the documented emergency bypass because stale QA deploy-summary SHA parity was the only failed pre-deploy gate; do not treat it as a clean no-bypass proof, but do treat the post-deploy runtime parity as current production evidence. The latest controlled DGFY production mutation UAT remains the June 13 run, which proved OTP delivery, fresh DGFY account registration, automatic company activation, IMS tenant-session handoff, onboarding item creation, DGFY-authenticated Storefront checkout, POS completion, inventory decrement from `10` to `9`, and DGFY account order visibility with ratings at or above `9.1`. Authenticated cashier/admin UAT remains the main human signoff gap.
