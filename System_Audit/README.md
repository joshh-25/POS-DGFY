...
3. Treat the latest production runtime code SHA evidenced in this workspace as `8b03dfea4f9615d665baa14df59a78de5c797a60`. The evidence is based on production deploy summary, health, asset parity, tenant schema sync, tenant index headroom, permission backfill, Storefront discovery reconciliation evidence from `/var/www/skupervisor/logs/deploy/deploy_20260610_234107.summary.txt`, production source/deploy-marker parity, and live no-company-token DGFY OTP request proof. The release used the auditable emergency no-staging bypass for stale QA deployed-head evidence, so `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` remains open until the QA parity proof is replaced or made deterministic.
