...
- Production deployment on 2026-06-08 deployed code target SHA `023e5f9ff0c71340cb9f92a9a7d7526920a62e05` using the documented emergency no-staging bypass because stale QA deployed-head evidence was the sole failed gate after QA smoke, rollback/restore docs, architecture, and compliance checks passed. Production remote `HEAD` and `.deploy-state/last_deployed_commit` matched the target SHA. Deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260608_024256.summary.txt` reported backend, IMS, POS, Storefront, public endpoints, frontend asset parity, tenant schema sync, and strict index-headroom checks passing.
...
3. Treat the current production code SHA as `023e5f9ff0c71340cb9f92a9a7d7526920a62e05` based on matching remote `HEAD`, deploy marker, production deploy summary, health, asset parity, and platform-admin tenant capability smoke. The release used the auditable emergency no-staging bypass for stale QA deployed-head evidence, so `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` remains open until the QA parity proof is replaced or made deterministic.
