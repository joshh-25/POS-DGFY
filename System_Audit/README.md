...
- The latest production runtime evidence available in this workspace is June 18, 2026 target SHA `4cf4c372c9eae91c6ead5932e19476f4c1b98e83`. Fetched deploy summary evidence `.tmp/release-gates/4cf4c372c9eae91c6ead5932e19476f4c1b98e83/qa_deploy_summary.txt` records production summary `/var/www/skupervisor/logs/deploy/deploy_20260618_114003.summary.txt` with `deployed_head`, `remote_head`, and `expected_commit` all matching the target SHA, frontend asset parity `pass`, tenant schema sync report output, tenant index headroom report output, `tenant_index_headroom_strict=0`, `migrations_changed=0`, and `total_changed_files=0`. Live `/api/v1/health` reports `services.observability.runtime_sha=4cf4c372c9eae91c6ead5932e19476f4c1b98e83` with source `deploy_state:last_deployed_commit`; database, Redis, runtime schema, schema indexes, billing telemetry, and observability are healthy. The no-staging release verdict passed with no emergency bypass after refreshed SHA parity evidence, and observability report-mode evidence passed trace-header round-trip and runtime-SHA matching. This deploy includes DGFY company access hardening, PR #20 POS/Storefront customer-flow work, post-merge DGFY security hardening, PR #18 Storefront discovery ranking, F&B mobile hero stabilization, restored Storefront customer dashboard layout, and the June 18 CSRF/POS terminal-lock remediation.
...
- DGFY company-access hardening is included in the deployed `4cf4c372c9eae91c6ead5932e19476f4c1b98e83` runtime chain. It tightens ownership transfer to `tenants.owner_dgfy_account_id`, demotes the former owner after transfer so leave-company rules are accurate, scopes DGFY account service failures away from generic tenant global toasts, makes development auto-login opt-in and excludes DGFY/POS lock entrypoints, expands the DGFY rendered-smoke script with strict-network mode, localhost HTTPS-forwarding, seeded text checks, and structured JSON evidence under `.tmp/rendered-qa/dgfy-access/`, and keeps the dedicated POS app locked until explicit terminal unlock before open-shift prompting. Seeded POS hardware UAT and strict local/staging DGFY access smoke remain useful before increasing the control matrix residual-risk rating.
...
- `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` - remediated 2026-06-18 with no-staging SHA parity passing without emergency bypass, live health runtime SHA proof, and observability report-mode evidence for target SHA `4cf4c372c9eae91c6ead5932e19476f4c1b98e83`
...
3. Treat the latest production runtime SHA evidenced in this workspace as `4cf4c372c9eae91c6ead5932e19476f4c1b98e83`. The evidence is based on fetched deploy summary `.tmp/release-gates/4cf4c372c9eae91c6ead5932e19476f4c1b98e83/qa_deploy_summary.txt`, production deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260618_114003.summary.txt`, live `/api/v1/health.services.observability.runtime_sha`, frontend asset parity, tenant schema sync report output, tenant index headroom report output, `migrations_changed=0`, no-staging release verdict pass without emergency bypass, and observability report-mode pass. The latest controlled DGFY production mutation UAT remains the June 13 run, which proved OTP delivery, fresh DGFY account registration, automatic company activation, IMS tenant-session handoff, onboarding item creation, DGFY-authenticated Storefront checkout, POS completion, inventory decrement from `10` to `9`, and DGFY account order visibility with ratings at or above `9.1`. Authenticated cashier/admin UAT remains the main human signoff gap.
