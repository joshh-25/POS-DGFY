...
- The latest production runtime evidence available in this workspace is June 18, 2026 target SHA `5caa201cb0468106fef859501181024f93746c49`. Fetched deploy summary evidence `.tmp/release-gates/5caa201cb0468106fef859501181024f93746c49/qa_deploy_summary.txt` records production summary `/var/www/skupervisor/logs/deploy/deploy_20260618_104004.summary.txt` with `deployed_head`, `remote_head`, and `expected_commit` all matching the target SHA, frontend asset parity `pass`, tenant schema sync report output, tenant index headroom report output, `migrations_changed=0`, and `total_changed_files=0`. Live `/api/v1/health` reports `services.observability.runtime_sha=5caa201cb0468106fef859501181024f93746c49` with source `deploy_state:last_deployed_commit`; database, Redis, runtime schema, schema indexes, billing telemetry, and observability are healthy. The no-staging release verdict passed with no emergency bypass, and observability report-mode evidence passed trace-header round-trip and runtime-SHA matching. This deploy includes DGFY company access hardening, PR #20 POS/Storefront customer-flow work, post-merge DGFY security hardening, PR #18 Storefront discovery ranking, F&B mobile hero stabilization, and restored Storefront customer dashboard layout.
...
- DGFY company-access hardening is included in the deployed `5caa201cb0468106fef859501181024f93746c49` runtime chain. It tightens ownership transfer to `tenants.owner_dgfy_account_id`, demotes the former owner after transfer so leave-company rules are accurate, scopes DGFY account service failures away from generic tenant global toasts, makes development auto-login opt-in and excludes DGFY/POS lock entrypoints, and expands the DGFY rendered-smoke script with strict-network mode, localhost HTTPS-forwarding, seeded text checks, and structured JSON evidence under `.tmp/rendered-qa/dgfy-access/`. Seeded POS hardware UAT and strict local/staging DGFY access smoke remain useful before increasing the control matrix residual-risk rating.
...
- `7.2-Release_evidence_depends_on_stale_or_bypassable_QA_paths.md` - remediated 2026-06-18 with no-staging SHA parity passing without emergency bypass, live health runtime SHA proof, and observability report-mode evidence for target SHA `5caa201cb0468106fef859501181024f93746c49`
