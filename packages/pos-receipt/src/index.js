...
function renderPosReceiptHtmlBase({ transaction, businessSettings = {}, receiptContract = null, paperWidth = '80mm' } = {}) {
  ...
  const itemRows = lines.map((line, index) => {
    ...
    const discountAmount = lineDiscountAmount(transaction, line);
    const grossTotal = discountAmount > 0 ? grossLineTotal(transaction, line) : netTotal;
    const discountRows = discountAmount > 0
      ? `<div class="grid line-adjustment"><span>${escapeHtml(discountRowLabel(transaction))}</span><span></span><span></span><span>${signedDiscount(discountAmount)}</span></div><div class="grid line-net-total"><span>NET TOTAL</span><span></span><span></span><strong>${money(netTotal)}</strong></div>`
      : '';
    return `<div class="line"><div class="grid"><strong>${escapeHtml(name)}</strong><span>${money(unitPrice(line))}</span><span>${formatQuantity(quantity(line))}</span><strong>${money(grossTotal)}</strong></div>${discountRows}${notes ? `<small>Modifiers: ${escapeHtml(notes)}</small>` : ''}</div>`;
  }).join('');
  const governedRows = discount ? [
    ['VAT Removed', discount.vat_removed], ['VAT-Exempt Amount', discount.vat_exempt_amount], ['Customer', discount.customer_name], ['Employee', discount.employee_name], ['Employee ID', discount.employee_id], ['Promo Code', discount.promo_code], ['Discount Type', upper(discount.discount_type)], ['Reason', discount.reason]
...
function renderThermalReceiptTextBase(input = {}) {
  ...
  const lineRows = lines.flatMap((line) => {
    ...
    if (discountAmount > 0) {
      rows.push(pair(discountRowLabel(transaction), signedDiscount(discountAmount)), pair('NET TOTAL', money(netTotal)));
    }
    return rows;
  });
  const summaryRows = [
