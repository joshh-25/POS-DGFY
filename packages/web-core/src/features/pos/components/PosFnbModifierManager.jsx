...
export default function PosFnbModifierManager({ groups = [], items = [], locations = [], busy = false, canManage = true, onCreate, onUpdate }) {
  ...
  return (
    <div className="grid min-w-0 max-w-full gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
      <section className="min-w-0 max-w-full overflow-hidden rounded-lg border border-slate-200 bg-white p-4">
        <div className="flex min-w-0 items-start justify-between gap-3">
          <div className="min-w-0"><h2 className="break-words text-lg font-black text-[#0F172A]">F&amp;B menu modifiers</h2><p className="break-words text-sm text-slate-500">Create restaurant choices, extras, and preparation options directly in this POS.</p></div>
          {canManage && <button type="button" onClick={() => { setEditingId(null); setDraft(emptyGroup()); }} className="shrink-0 rounded-lg border px-3 py-2 text-sm font-semibold"><Plus className="mr-1 inline h-4 w-4" />New group</button>}
        </div>
      ...
      <section className="min-w-0 max-w-full overflow-hidden rounded-lg border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 min-w-0 space-y-3">
          <input aria-label="Group name" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="Group name" className="h-10 w-full min-w-0 max-w-full rounded-lg border px-3 text-sm" />
          ...
          {locations.length > 0 && <div className="min-w-0"><p className="text-xs font-semibold text-slate-600">Location availability</p><div className="mt-2 grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-2">{locations.map((location) => { const row = draft.location_availability.find((entry) => Number(entry.location_id) === Number(location.location_id)); return <Toggle key={location.location_id} label={location.name} checked={row?.is_available !== false} onChange={(value) => setGroupLocation(location.location_id, value)} />; })}</div></div>}
           <div className="min-w-0 space-y-2"><div className="flex min-w-0 items-center justify-between gap-2"><p className="text-sm font-semibold">Options</p><button type="button" onClick={() => setDraft({ ...draft, options: [...draft.options, emptyOption()] })} className="shrink-0 text-xs font-semibold text-teal-700">+ Add option</button></div>
             <div data-testid="fnb-modifier-options-list" className="min-w-0 max-h-96 space-y-2 overflow-y-auto pr-1">
               {draft.options.map((option, index) => <div key={option.modifier_option_id || `new-${index}`} className="min-w-0 overflow-hidden rounded-lg border p-3"><div className="grid min-w-0 grid-cols-[minmax(0,1fr)_5.5rem_auto] items-center gap-2"><input aria-label={`Option ${index + 1} name`} value={option.name} onChange={(e) => updateOption(index, { name: e.target.value })} placeholder="Option name" className="h-9 min-w-0 w-full rounded border px-2 text-sm" /><input aria-label={`Option ${index + 1} price`} type="number" step="0.01" value={option.price_delta} onChange={(e) => updateOption(index, { price_delta: e.target.value })} className="h-9 w-full min-w-0 rounded border px-2 text-sm" /><button type="button" aria-label={`Remove option ${index + 1}`} onClick={() => removeOption(index)} className="shrink-0"><X className="h-4 w-4" /></button></div><select aria-label={`Option ${index + 1} inventory link`} value={option.sku_item_id} onChange={(e) => updateOption(index, { sku_item_id: e.target.value })} className="mt-2 h-9 w-full min-w-0 max-w-full rounded border px-2 text-xs"><option value="">No inventory link</option>{itemOptions.map((item) => <option key={item.item_id} value={item.item_id}>{item.name}</option>)}</select><div className="mt-2 grid min-w-0 grid-cols-1 gap-1 sm:grid-cols-2"><Toggle label="Default" checked={option.is_default} onChange={(value) => updateOption(index, { is_default: value })} /><Toggle label="Sold out" checked={option.is_sold_out} onChange={(value) => updateOption(index, { is_sold_out: value })} /><Toggle label="POS" checked={option.visible_in_pos} onChange={(value) => updateOption(index, { visible_in_pos: value })} /><Toggle label="Online" checked={option.visible_in_storefront} onChange={(value) => updateOption(index, { visible_in_storefront: value })} /></div></div>)}
             </div>
           </div>
          <button type="button" onClick={submit} disabled={busy || !draft.name.trim() || !draft.options.some((option) => option.name.trim())} className="flex min-w-0 w-full items-center justify-center gap-2 rounded-lg bg-teal-700 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50"><Save className="h-4 w-4" />{editingId ? 'Save changes' : 'Create group'}</button>
        </div>
        </>
        )}
      </section>
