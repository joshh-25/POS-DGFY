...
- The DGFY POS surface split is tracked in `docs/testing/pos-readiness-status.md` and `docs/compliance/impact-declarations/2026-05-30-dgfy-pos-surface-split.md`. Standalone POS owns cashier terminal routing, while SKUpervisor `/pos` owns admin/mode panels and reporting handoff; local release gates, `npm run smoke:pos-terminal-ui`, and shared checkout payload/fee contract tests now pass, but production promotion still requires human UAT and exact deployed-SHA verification.
