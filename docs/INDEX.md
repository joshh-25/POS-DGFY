...
- Inventory item/product create-edit flows are covered by `docs/features/CUSTOMER_ACCESS_MODES_AND_INVENTORY_DISPLAY.md` where they intersect POS/Storefront catalog setup. They expose guarded footer actions and a top-right close control across create/edit modes, keep draft save separate from finalization, preserve branch/gallery setup on save paths, retain internal Storefront catalog image field names while using `Item image` in merchant-facing copy, and share the single-item POS Upload Image / Storefront Add Item Images controls through the same appendable item-image gallery while keeping POS and Storefront visibility separate.
