...
4. `frontend/src/features/pos/components/TerminalOperationsWorkspace.jsx` (`ItemsWorkspace`):
   ...
   - Create and edit forms now use an editable ARIA combobox instead of a browser-owned `<input list>` suggestion field. It opens with all active categories available for explicit selection and filters only after typing; authorized category managers can still type a new category name to create it on save.
