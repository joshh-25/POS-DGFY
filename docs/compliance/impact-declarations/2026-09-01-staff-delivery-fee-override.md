...
3. `apps/dgfy-api/src/modules/pos/usecases/deliveryFeeOverrideUseCases.js` (**new**) --
   ...
   recomputes `total_amount` (delta-adjusted, floored at 0) and `balance_due`
   (`max(0, total_amount - amount_paid)`, matching `storeUseCases.js`'s own downpayment-balance
   formula), and writes one `audit_logs` row in the same DB transaction. A same-value resubmit is a
   no-op: no write, no audit row, still a success response (the client always sends the absolute
   target fee, never a delta, so a retried request cannot double-apply).
4. `apps/dgfy-api/src/modules/pos/controllers/posHandlers.js` -- new `overrideDeliveryFee` handler.
   Deliberately not routed through `posMutationUser`/`requirePairedTerminal` (unlike void/refund) --
   uses `req.user` directly, since this is an order-level permission check, not a terminal
   cash-handling action.
...
3. **Recomputes balance due correctly.** `total_amount` is delta-adjusted from the existing value
   (`existing.total_amount + (new_fee - previous_fee)`, floored at 0) rather than re-derived from
   the full totals formula, so every other component (subtotal, discount, service fee) is left
   exactly as checkout computed it. `balance_due` is then `max(0, total_amount - amount_paid)`,
   the same formula `storeUseCases.js` uses for the downpayment balance split. Since the override
   is only reachable while `payment_status === 'unpaid'`, `amount_paid` is 0 in the reachable case,
   but the formula does not assume that -- it is computed generically and self-corrects if that
...
4. **Audited with before/after + actor.** Confirmed against the codebase before implementing,
   ...
   cash-collection). Pinned by test: the audit row's `changes` payload is asserted to carry both
   before and after values plus `payment_status_at_override`.
...
See the `verification_evidence` front-matter field for the itemized list. Summary: 11 new unit
tests for the use case (all payment-status/order-method/reason/actor/negative-fee/no-op/not-found
branches), all passing; 32 existing tests across cash-refund, mode-role-preset, and
