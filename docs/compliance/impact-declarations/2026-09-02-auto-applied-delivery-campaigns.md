...
- **The `delivery_voucher_feedback` response contract changed on both the cart-quote and checkout
  ...
  .enteredDeliveryVoucherCode` (the resolved canonical code, populated on both the code-entered and
  auto-applied paths) rather than echoing `payload.delivery_voucher_code` back, which would be empty
  on an auto-applied order and defeat the ticket's own acceptance evidence ("fee shows as 0 in the
  cart drawer" is not useful without saying which campaign did it). Additive `auto_applied` and
  `label` fields, for the storefront UI (#1391, not built here) to render an auto-applied campaign
  distinctly from a typed one.
...
3. **QRPh pin round-trip coverage is narrower than the plan's own §9.6** — the backward-compatibility
   case (an in-flight pin missing `autoAppliedVoucherId`) is verified indirectly, via
   `storeCheckoutDeliveryFeePin.unit.test.js`'s existing 27 cases staying green (none of its fixtures
   carry the new key, so every one of them exercises the `undefined`-treated-as-`null` path already).
   A dedicated webhook-finalize test exercising a *present* `autoAppliedVoucherId` end to end (the
   full `finalizePaidCommerceSession.js` plumbing, module-mocking `storeCheckoutUseCase` the way that
   file's own "layer 2" already does) was not written in this pass — flagged for the reviewer rather
