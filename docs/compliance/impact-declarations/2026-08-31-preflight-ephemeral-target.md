...
`compliance` only, per the `surfaces:` front matter key. Concretely: the compliance preflight sweep
workflow (`.github/workflows/compliance-preflight-sweep.yml`), its ephemeral-instance fixture
scripts (`apps/dgfy-api/scripts/seed-preflight-fixture.js`,
`apps/dgfy-api/scripts/teardown-preflight-fixture.js`), the reconciliation script
(`scripts/reconcile-preflight-declarations.js` + its test), the `build-preflight-request.js`
surface-filter/evidence-truncation additions, and the `tenantModelFactory.js`
`NON_TENANT_MODEL_EXPORTS` fix (#1233). No authorization, shift, payment, or document-classification
surface is touched — see "Why `minor`" above for the reasoning.

## Compliance Preconditions

`NOT-EXECUTED-*` on `preflight_request_ref` at filing time, disclosed honestly per this repo's own
established precedent: this PR is what *builds* the mechanism that runs live preflight — running it
against itself would be circular, the same reason `check-compliance-impact.js`'s own PRs don't
preflight themselves. Classification is `minor`, so the preflight-key requirement
(`PREFLIGHT_REQUIRED_CLASSIFICATIONS`) does not apply to this declaration in the first place; the
`preflight_*` front matter keys above were filled in later, once the mechanism this PR built ran its
own live sweep against this declaration post-merge — see `preflight_result: no_breach` and
`preflight_request_ref` in the front matter, and "Verification Evidence" below for what that run
actually did.

## Verification Evidence

See `verification_evidence` in the front matter for the full list. Summarized: a real declaration
from the current batch was run through the real preflight endpoint via this exact pipeline (isolated
docker network, mysql+redis+dgfy-api provisioned fresh, real `provisionTenant()` tenant creation,
real `POST /api/v1/auth/login`, real `POST /api/v1/compliance/preflight`), returning a real
`no_breach`/`can_proceed: true` — first against a throwaway spike script, then repeated against the
final production `seed-preflight-fixture.js`. `build-preflight-request.test.js` (12/12, 5 new) and
`reconcile-preflight-declarations.test.js` (6/6, including that a failing verdict is never
reconciled) both pass. A manual reconcile run against a real declaration copy confirmed only the two
intended front-matter lines changed. `node --check` on every changed/new `.js` file; YAML structural
validation and `bash -n` on every `run:` block in the rewritten workflow. `npm run check:compliance`
confirmed clean at filing time (no mechanical requirement for this diff, since the workflow/script
paths this diff touched don't match `COMPLIANCE_SENSITIVE_RULES`). Once this PR merged to `develop`,
the sweep it adds auto-triggered on the very next declaration to land and, from that point on, every
future declaration converts its own `NOT-EXECUTED-*` automatically — this was the last declaration in
this repo's history expected to carry that placeholder by hand.
