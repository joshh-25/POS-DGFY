...
Regulatory (surface-driven minimum). The change itself touches compliance-sensitive POS route and layout files to restore lazy-loaded bundle boundaries, and changes module loading and release-budget enforcement only. Classification is escalated to `regulatory` to satisfy the repository-wide compliance surface floor for compliance-sensitive files changed in this same branch/PR; this slice does not itself modify compliance policy evaluation logic.
