...
The release does not change fiscal receipt issuance, POS checkout authorization, payment authorization, tenant capability enforcement, or platform-admin approval policy. Storefront gallery cleanup is tenant-scoped and removes only omitted local `storefront-assets/<tenant>/...` paths after settings persistence succeeds.
