...
verification_evidence: npm run build:pos -- real Vite build, OK,npm run build:skupervisor -- real Vite build, OK (apps/dgfy-ims lazily imports the same TerminalPage.jsx tree via packages/web-core),packages/web-core/src/features/pos/utils/__tests__/queueRunDropAssignment.test.js -- actually executed (Vitest via apps/dgfy-ims), 9/9 passing,packages/web-core/src/features/pos/__tests__/deliveryRunSplitViewDnd.behavior.test.jsx -- actually executed (Vitest via apps/dgfy-ims), 8/8 passing,packages/web-core/src/features/pos/__tests__/deliveryRunBulkAssign.behavior.test.jsx -- re-run after this phase's extraction and signature change, all passing (regression guard),packages/web-core/src/features/pos/__tests__/deliveryRunsWorkspace.behavior.test.jsx -- re-run, all passing (regression guard),packages/web-core/src/features/pos/__tests__/terminalViewModeContracts.test.js -- updated for the extraction (assertions targeting moved content now read IncomingQueueOrderList.jsx) and re-run, all passing,full apps/dgfy-ims Vitest suite -- re-run in full, 314 files / 1966 tests passing, no regressions,npm run check:architecture -- OK,npm run check:adr -- OK,npm run lint:docs -- OK,npm run check:compliance -- confirmed to fail first (listing every touched file below), then pass once this declaration was added,PR #1305 review fix (RF-1/RF-2) -- npm run build:pos OK, npm run build:skupervisor OK, deliveryRunSplitViewDnd.behavior.test.jsx + queueRunDropAssignment.test.js re-run 17/17 passing, full apps/dgfy-ims Vitest suite re-run 314 files / 1966 tests passing,PR #1305 re-review rebase (RF-5/RF-6/RF-7) -- renumbered Phase 229 to 230 (#1291 kept 229 on develop), resolved real conflicts in IMPLEMENTATION_PHASE_LEDGER.md and TerminalOperationsPanels.jsx, ported #1291's run-member fulfillment gate into IncomingQueueOrderList.jsx and extended incomingQueueRunMemberFulfillmentGate.behavior.test.jsx with two split-view cases confirming the gate in both view modes (8/8 passing), npm run build:pos OK, npm run build:skupervisor OK, npm run check:architecture/check:adr/lint:docs OK, npm run check:compliance PASS post-merge (GITHUB_BASE_REF=develop), full apps/dgfy-ims Vitest suite re-run 315 files / 1985 tests passing, no regressions,PR #1305 second rebase (#1288/PR #1304 merged concurrently, taking Phase 230) -- renumbered again to Phase 231, merged origin/develop a second time resolving real conflicts in IMPLEMENTATION_PHASE_LEDGER.md, TerminalOperationsPanels.jsx and terminalViewModeContracts.test.js; #1288 reverted the tab view's card rendering to inline (using its own new buildIncomingQueueOrderActions shared action-builder, which already carries #1291's ported gate) so the tab view now uses develop's inline block again while the split view keeps this phase's IncomingQueueOrderList.jsx extraction; consolidated the duplicate formatting util onto #1288's incomingQueueOrderFormatting.js, deleting this phase's own orderListFormatting.js; restored three component imports (DeliveryAssignmentControl, DeliveryAddressEditControl, QueueOrderSelectCheckbox) dropped by the extraction but still needed by the reinstated inline tab-view block, npm run build:pos OK, npm run build:skupervisor OK, npm run check:architecture/check:adr/lint:docs OK, full apps/dgfy-ims Vitest suite re-run 316 files / 1993 tests passing, no regressions,PR #1305 third rebase (#1290/PR #1303 merged concurrently, taking Phase 231) -- renumbered again to Phase 232, merged origin/develop a third time resolving real conflicts in IMPLEMENTATION_PHASE_LEDGER.md, TerminalOperationsPanels.jsx and terminalViewModeContracts.test.js; combined this phase's split-view viewport/drag state with #1290's runFilter/visibleIncomingOrders correctness fix (selectedEligibleOrders re-derived against the filtered list) and its mode-flip reset; switched the split view to render visibleIncomingOrders instead of sortedIncomingOrders so a filtered-out order cannot be dropped onto a run while hidden from the tab view; updated the split view's QueueRunAssignBar call site to pass the new runs/runsLoading/runsError/hiddenCount props (#1290 lifted the bar's own fetch into a shared useDeliveryRunOptions hook) -- missed on the first resolution pass and caught by deliveryRunSplitViewDnd.behavior.test.jsx's \'still submits via QueueRunAssignBar unchanged\' case failing (0 calls instead of 1), fixed and re-verified 8/8 passing, npm run build:pos OK, npm run build:skupervisor OK, npm run check:architecture/check:adr/lint:docs OK, full apps/dgfy-ims Vitest suite re-run 318 files / 2020 tests passing, no regressions
