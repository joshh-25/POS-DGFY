...
7. **A secondary membership to an inactive or soft-deleted folder can never leak that folder's name
   onto the public storefront catalog (fixed in PR #1579 review round 1, Codex RF-1).** Folder
   deletion is soft -- `itemRepository.deleteFolder` sets `is_active: false` and `deleted_at`, and
   leaves any `item_folder_memberships` rows pointing at that folder in place (this is exactly the
   under-count ADR 0080 Consequences item 4 already names: a merchant deleting a folder sees an
   under-count of affected items in the pre-delete warning until a later phase fixes that warning,
   but the memberships themselves persist regardless). The initial version of this PR's
   `attachSecondaryCategories` queried `ItemFolder` by `folder_id` alone, with no `is_active`/
   `deleted_at` scoping, so a stale membership's folder name would still resolve and appear on the
   live public catalog. **Fix:** the `ItemFolder.findAll` query now adds `is_active: true,
   deleted_at: null`, and the membership-to-row mapping only emits a `{folder_id, folder_name}`
   entry for a `folder_id` the (now-scoped) query actually returned -- a membership whose folder
   fails that scope is dropped from `secondary_categories` entirely, not returned with a null name.
   Pinned by three new tests: an inactive folder's membership dropped (with the query's `where`
   asserted to carry `is_active: true`/`deleted_at: null`), a soft-deleted folder's membership
   dropped the same way, and a sibling active membership kept when another membership on the same
   item points at an inactive folder (proving the exclusion is per-membership, not all-or-nothing
   per item).
...
- 3 new test files, 17/17 tests passing: repository-level projection behavior (8, including the
  RF-1 active/soft-deleted-folder exclusion), use-case-level threading into the serialized response
  (3), and the new ADR 0080 Decision 1 static regression guard (6).
