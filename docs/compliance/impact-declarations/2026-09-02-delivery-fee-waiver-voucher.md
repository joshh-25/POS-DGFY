...
## PR #1389 review fixup (RF-1)

`pr-reviewer` (Codex GPT-5.6 Luna) found that the delivery-axis resolution guard in
`resolveCheckoutContext` (`storeUseCases.js`) was gated on `delivery.baseFee > 0`, in addition to
`normalized.delivery_voucher_code && orderMethod === 'delivery'`. For a delivery order whose
resolved base fee is legitimately zero (free-delivery mode — see "What this phase does and does not
do" above, `feeConfig.mode === 'free'` in `resolveStoreDeliveryFee`), the entire block — validation,
axis-mismatch checking, AND redemption — was silently skipped: an item-targeted code entered in
`delivery_voucher_code` passed through instead of failing closed with
`VOUCHER_BENEFIT_TARGET_MISMATCH`, and a validly-entered code on the pinned QRPh/webhook-finalization
replay path never received its required exactly-once redemption.

Fixed by dropping the `delivery.baseFee > 0` condition — the guard is now
`normalized.delivery_voucher_code && orderMethod === 'delivery'` only. `orderMethod === 'delivery'`
alone already distinguishes "has this axis at all" (a pickup order still skips the block entirely,
never burning a redemption) from "the axis resolved to zero" (a free-mode delivery order still runs
the block; the resulting waiver is naturally `0` since `voucherBenefitPolicy.js`'s own
`benefitBaseCentavos` clamp already treats `0` as a legal, distinct-from-`null` base — see that
module's own comment). No domain-layer change was needed; this was purely a call-site guard defect.

New test coverage added to `storeCheckoutDeliveryWaiverDualAxis.unit.test.js` (see
`verification_evidence`): a free-mode zero-base delivery order with a valid delivery-targeted code,
the same order with an item-targeted code (mismatch), and a pinned/webhook-replay free-mode
zero-base order (exactly-once redemption). The pickup-order negative case was already covered by an
existing test in the same file and needed no change, since its guard is unaffected by this fix.
