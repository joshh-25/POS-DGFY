...
This declaration covers the inventory wizard and POS catalog image handling change that makes the POS image upload control use the shared item-image gallery when that gallery uploader is available. POS catalog rows now fall back to the shared Storefront catalog primary item image when no legacy POS-only image is configured.
...
- POS Upload Image and Storefront Add Item Images now write through the same item-image gallery path when the shared uploader is wired.
...
4. Rollback can restore the previous independent POS image upload behavior without schema rollback.
