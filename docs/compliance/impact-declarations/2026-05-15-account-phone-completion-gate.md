...
verification_evidence: npm --prefix backend test -- --runTestsByPath tests/auth.test.js tests/userHandlers.transport.test.js,npm --prefix frontend exec vitest run src/services/__tests__/api.globalErrors.test.js,npm run check:architecture,npm run check:docs,npm run build:skupervisor,npm --prefix backend run verify:phone-rollout
rollback_note: Revert the auth phone-completion gate, cache invalidation hook, and frontend redirect while keeping nullable phone columns and existing registration/profile validation intact.
...
3. Other authenticated tenant work must fail closed until a valid phone number is stored.
4. Successful phone completion must invalidate stale auth cache state immediately so the user is not held in a blocked state after correction.
5. Outstanding legacy-user gaps must remain visible to operators until rollout closure is complete.

## Verification Evidence

- `npm --prefix backend test -- --runTestsByPath tests/auth.test.js tests/userHandlers.transport.test.js`
