...
| Engineering | ENG-01 Compliance-sensitive diff declaration | CI/pre-commit fail when sensitive changes lack high-quality declaration | `IMPACT_DECLARATION_REQUIRED` | `scripts/check-compliance-impact.js`, `.husky/pre-commit`, `.github/workflows/ci.yml` | `npm run check:compliance` |
