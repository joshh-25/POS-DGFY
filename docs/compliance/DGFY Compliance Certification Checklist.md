...
- [x] Reset Counter
  ...
  - References: `backend/src/modules/pos/usecases/posUseCases.js`, `backend/src/routes/pos.js`, `backend/tests/posReadings.usecase.test.js`.
...
- [x] Tamper-Proofing (No Bypass Modes)
  ...
  - References: `backend/src/modules/compliance/policy/compliancePolicyEngine.js`, `backend/src/modules/pos/usecases/posUseCases.js`, `frontend/src/features/pos/components/ReceiptPrintView.jsx`, `backend/tests/compliancePolicyEngine.test.js`.
...
- [x] VAT / Non-VAT Segregation
  ...
  - References: `backend/src/modules/pos/usecases/posUseCases.js`, `frontend/src/features/pos/components/ReceiptPrintView.jsx`, `backend/tests/posSalesReconciliation.db.integration.test.js`, `frontend/src/features/pos/__tests__/receiptContractConformance.contract.test.js`.
...
- [x] Z-Reading Report
  ...
  - References: `backend/src/modules/pos/usecases/posUseCases.js`, `backend/src/models/PosZReadingSnapshot.js`, `backend/tests/posReadings.usecase.test.js`.

- [x] X-Reading Report
  - Evidence: `GET /pos/x-reading/current` returns intraday summary and counter state.
  - References: `backend/src/modules/pos/usecases/posUseCases.js`, `backend/src/routes/pos.js`, `docs/api/specification.md`, `backend/tests/posHandlers.transport.test.js`.
...
- [x] System-Generated Books of Accounts
  ...
  - References: `backend/src/services/reportService.js`, `backend/src/modules/reports/controllers/reportHandlers.js`, `backend/tests/reportHandlers.transport.test.js`, `docs/compliance/submission/filing-instructions.md`.
...
- [x] Backend Discount Reporting (Senior/PWD/National Athlete + name/ID)
  ...
  - References: `backend/src/modules/pos/usecases/posUseCases.js`, `backend/src/services/reportService.js`, `backend/tests/reportHandlers.transport.test.js`.

## 3. Data Privacy & Security (NPC Compliance)

- [x] Data Encryption
  - Evidence: Production HTTPS enforcement + HSTS and documented AES-256 at-rest controls.
  - References: `backend/src/server.js`, `backend/tests/securityTransport.middleware.test.js`, `docs/compliance/submission/backup-disaster-recovery-plan.md`, `docs/compliance/evidence/drills/latest-encryption-verification.json`.

- [x] Access Control (RBAC)
  - Evidence: Sensitive-action RBAC matrix maps to route/usecase enforcement and admin compliance actions.
  - References: `docs/compliance/evidence/rbac-sensitive-action-matrix.md`, `backend/src/middleware/auth.js`, `backend/src/routes/compliance.js`, `backend/src/routes/pos.js`, `backend/src/routes/adminTenants.js`, `backend/tests/rbacRouteCoverage.contract.test.js`.

- [x] Immutable Audit Logs
  - Evidence: DB triggers reject update/delete on `tenant_compliance_audit_logs`; readiness checks validate trigger presence.
  - References: `backend/migrations/20260408000003-harden-compliance-audit-immutability.cjs`, `backend/src/modules/compliance/repositories/complianceRepository.js`, `backend/tests/complianceRepository.documentaryReadiness.test.js`.
...
- [x] Breach Notification Tooling
  ...
  - References: `backend/src/modules/compliance/usecases/complianceUseCases.js`, `backend/src/routes/adminTenants.js`, `frontend/Pages/admin/TenantManager.jsx`, `backend/tests/complianceSecuritySignal.usecase.test.js`, `backend/tests/complianceSecurityIncidents.usecase.test.js`.
...
- [x] External Payment Handoff
  ...
  - References: `backend/src/modules/compliance/policy/compliancePolicyEngine.js`, `backend/src/middleware/compliancePolicy.js`, `frontend/src/features/pos/pages/TerminalPage.jsx`, `frontend/src/features/pos/components/TerminalPageLayout.jsx`, `frontend/src/features/settings/settingsDeepLink.js`, `frontend/src/features/settings/__tests__/settingsDeepLink.contract.test.js`, `frontend/src/pages/__tests__/Settings.deepLinking.integration.test.jsx`, `frontend/src/features/compliance/__tests__/ComplianceProgramPanel.integration.test.jsx`, `backend/tests/compliancePolicyEngine.test.js`.
...
- [x] Software Specifications Document
  ...
  - References: `docs/compliance/submission/software-specification-dgfy.md`, `backend/tests/complianceRepository.documentaryReadiness.test.js`, `docs/compliance/evidence/open-controls-matrix.md`.
...
- [x] Data Backup & Disaster Recovery Plan
  ...
  - References: `docs/compliance/submission/backup-disaster-recovery-plan.md`, `docs/compliance/evidence/drills/latest-restore-drill.json`, `backend/tests/complianceRepository.documentaryReadiness.test.js`.
