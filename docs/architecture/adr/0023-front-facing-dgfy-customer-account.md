...
11. Storefront browsing remains public, but checkout and service booking completion require an authenticated DGFY account session before the customer-details step. Storefront flows must preserve the current cart or booking draft through the auth handoff, return the customer to step 1 of the active flow after auth, render read-only account identity from the DGFY session instead of asking for phone/email again, and keep all resulting activity account-owned from creation time.
...
2. Guest browsing remains allowed, but guest checkout and guest booking completion are no longer part of the forward storefront flow.
...
4. Existing Storefront checkout/order tests proving the account-required gate, post-auth draft resume, and DGFY-linked store-customer behavior do not regress, plus Services booking tests proving authenticated-account linkage.
