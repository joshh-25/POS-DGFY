...
21. After active auto-standard tenant provisioning, the registration UI immediately exchanges the signed-in DGFY account plus accepted founder membership for a normal SKUpervisor tenant session through `POST /api/v1/dgfy/auth/tenant-session`, then redirects the founder to IMS/onboarding without asking for SKUpervisor credentials again. The endpoint must only issue a session for an active accepted membership linked to the authenticated DGFY account and an active tenant user. Manual SKUpervisor login with prefilled DGFY email and company token remains only a failure fallback when the tenant-session exchange cannot be completed.
