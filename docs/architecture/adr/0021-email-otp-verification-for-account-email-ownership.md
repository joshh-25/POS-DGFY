...
7. Public DGFY account registration requests a `dgfy_account_verification` OTP through the public email-OTP endpoint before account creation. The registration mutation consumes that OTP, creates the account, and sets `email_verified_at` in the same successful account-creation flow.
...
11. Invitation-acceptance OTP requests resolve tenant context from `invitation_token` when no `x-company-token` is present, matching token-only invite validation and acceptance links.
