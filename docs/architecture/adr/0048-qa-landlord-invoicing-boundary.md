...
- Persist centavo integers and immutable seller, buyer, service, and recipient snapshots. One original invoice exists per registration application, enforced by a nullable original-only application key with a database unique index so replacements remain possible.
...
- Discarding a draft transitions it to an auditable terminal state. It does not delete the invoice row or its append-only events, and it releases the original-only application guard so a corrected draft can be started.
