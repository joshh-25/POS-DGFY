...
1. Platform-admin force downgrade
   ...
   - Requires reason and immutable compliance audit event. `[binding]`
...
1. Add tenant override/revert tracking columns. `[snapshot]`
2. Add cycle-version columns for one-per-cycle enforcement. `[snapshot]`
3. Replace strict no-downgrade DB trigger with controlled-exception trigger that only permits downgrade when governed markers are present. `[binding]`
4. Extend compliance audit event taxonomy with:
   - `mode_force_non_compliant`
   - `mode_revert_non_compliant` `[default]`
5. Harden controlled-exception trigger semantics (`20260422000002-harden-compliance-downgrade-controls.cjs`):
   ...
   - tenant revert path must persist current `compliance_cycle_version` to `compliance_revert_last_cycle_version`. `[binding]`
6. Enforce fail-closed audit durability for downgrade operations:
   ...
   - fallback-only audit persistence is treated as a transactional failure. `[binding]`
