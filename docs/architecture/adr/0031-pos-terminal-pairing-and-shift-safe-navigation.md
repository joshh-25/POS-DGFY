...
proposed a second tenant-local cashier credential system. That would conflict
with ADR 0028, duplicate identity lifecycle, and weaken membership revocation.
The valid requirement is terminal possession proof, not another user identity.

## Decision

1. Keep DGFY membership as the primary identity and company-access authority.
2. Reject tenant-local cashier creation, cashier passwords, cashier login, and
   cashier persistence introduced by PR #25.
...
The Settings module owns terminal registry secret storage. The POS module owns
pairing verification and mutation guards. DGFY owns account and company
membership. Tenant-local users remain authorization profiles, not a new
credential authority. No production deployment behavior changes in this ADR.
