...
- Initiative: Downpayment & partial payment checkout (epic #815). Issue #848, Phase 143 of the
  epic's phase sequence, continuing this ledger's numbering. Runs in parallel to Phase 142 (#823),
  whose merge is separately, deliberately paused pending review — cut from a fresh `origin/develop`
  worktree rather than #823's branch, and shares no files with it.
...
- Does not gate or depend on Phase 142 (#823) — different app (POS settings vs. storefront
  checkout), different files, cut from a separate `origin/develop` worktree.
