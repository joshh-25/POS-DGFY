...
- Weekly Storefront business hours captured during primary-location setup; these hours are editable later in Settings and gate Storefront checkout availability
...
- Primary storefront location pin through the shared IMS MapLibre picker, with click-to-place, drag-to-adjust, browser geolocation, coordinate fields, delivery-radius preview, and first-party address-label autofill. The shared picker is also used by Settings > Storefront location editing, opens over Iloilo City, Philippines when no saved pin exists, and uses the shared Storefront MapLibre basemap style so merchants can see street context while address lookup remains on the first-party reverse-geocode endpoint. Iloilo City is the default camera view only, not a default saved merchant pin; onboarding and Settings reject missing coordinates, `0,0`, and browser geolocation results outside the Philippines before saving a merchant storefront location. Onboarding only exposes interactive pin controls after the merchant enables searchable storefront visibility and does not allow `Adjust Pin` to enter moving mode without a visible selected marker.
...
3. Storefront business-hours changes from onboarding or Settings refresh discovery so the public hours label stays current.
4. Keep storefront page editable through existing Settings after completion.
