...
- F&B onboarding item creation accepts `Ingredient` as the valid preset and normalizes legacy `raw material` / `raw_material` input at the onboarding boundary only. Persisted F&B ingredient rows use `mode_item_preset=ingredient` and `category=raw_material`; completion readiness still requires `menu_item` or `packaged_beverage`.
