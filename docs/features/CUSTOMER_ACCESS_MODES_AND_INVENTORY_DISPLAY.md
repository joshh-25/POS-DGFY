...
- Merchant image upload is currently replacement-first: selecting multiple Storefront item images replaces the ordered gallery and makes the first selected image primary. Per-image deletion, append-only upload, drag reorder, and explicit primary-image selection are deferred gallery-management enhancements and must not be assumed available from the current UI.
