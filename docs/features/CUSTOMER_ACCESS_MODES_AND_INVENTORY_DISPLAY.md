...
- The shared IMS MapLibre pin picker used by onboarding and Settings has explicit locked and adjust modes. Settings opens locked and requires `Adjust Pin` before map click, marker drag, or browser-geolocation overwrite can change coordinates. Onboarding opens in adjust mode only after searchable storefront visibility is enabled and no valid saved pin exists. Coordinates remain the source of truth for pin placement and are still saved when reverse geocoding is unavailable or returns no usable address. Iloilo City, Philippines is the default camera view when no saved pin exists, not an implicit saved merchant pin. IMS merchant-store pins must be finite Philippines coordinates; missing values, `0,0`, and out-of-Philippines browser geolocation results are rejected before tenant-location saves. Browser geolocation shows reported accuracy and must not claim guaranteed precision. Address suggestions come from first-party `/api/v1/geo/reverse-geocode` with `provider="dgfy-ph-local"`, precision, PSGC provenance, and optional PSGC code metadata; the picker may fill an empty address field but must not overwrite merchant-edited text unless the merchant applies the suggestion.
...
- 2026-06-21 hotfix: `MapPinPicker` owns an immediate local draft pin after map click, geolocation success, or `Adjust Pin`, then reconciles with parent latitude/longitude props. Onboarding does not render interactive pin controls while searchable storefront visibility is off, preventing a no-op map callback.
- 2026-06-21 follow-up: Settings now stays locked until `Adjust Pin`, onboarding can start in adjust mode for a first searchable pin, browser geolocation shows accuracy feedback, PH-local reverse geocode returns `dgfy-ph-local` precision metadata without `Near ...` labels, and `storefront_hours` supports multiple intervals per day.
- Targeted frontend suites passed from `frontend/`: `npm exec vitest run src/components/maps/__tests__/MapPinPicker.maplibre.test.jsx src/features/settings/__tests__/storefrontBusinessHours.test.js src/features/settings/__tests__/StorefrontBusinessHoursScheduler.test.jsx src/features/onboarding/__tests__/OnboardingSetupModal.behavior.test.jsx src/pages/__tests__/Settings.deepLinking.integration.test.jsx --pool=threads` (68 tests).
