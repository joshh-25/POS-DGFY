...
- Tenant merchant IDs may be generated by PayMongo child-account APIs and stored as tenant `provider_merchant_id`; the DGFY parent/platform merchant ID for `PAYMONGO_DGFY_MERCHANT_ID` must not be inferred from a tenant child account.
- Live QR Ph split checkout is blocked unless `PAYMONGO_LIVE_PLATFORM_SPLIT_CONFIRMED=true` or `PAYMONGO_PLATFORM_SPLIT_CONFIRMED=true` is explicitly set after PayMongo confirms the parent merchant ID and `split_payment` marketplace capability.
- Current external provider status as of June 25, 2026: PayMongo support replied that Linked Accounts is not configured for the account and self-service onboarding is still under development. This supersedes earlier dashboard ambiguity; DGFY must treat PayMongo marketplace split settlement as unavailable for live customer traffic until PayMongo explicitly enables the account.
...
- Sandbox/internal work may continue with API-generated tenant child merchant IDs, but production/live checkout must remain disabled until PayMongo confirms the parent/platform split capability. PayMongo support's June 25, 2026 response states that Linked Accounts is not configured for the account and self-service onboarding is still under development, so this is an explicit external go-live blocker.
...
6. Set `PAYMONGO_LIVE_PLATFORM_SPLIT_CONFIRMED=true` only after PayMongo confirms the parent merchant/account ID and live `split_payment.transfer_to` plus fixed `split_payment.recipients[].merchant_id` capability.
7. Restart the production backend and re-probe the endpoint before running any live canary payment.
8. Use `PAYMONGO_WEBHOOK_ENDPOINT_URL=https://skupervisor.surebizcorp.com/api/v1/commerce-payments/paymongo/webhook npm --prefix backend run verify:paymongo:webhook` from a shell that supports inline env, or pass the URL as the script argument from PowerShell.
...
- Production live customer use remains blocked because PayMongo support confirmed Linked Accounts is not configured for the account. Do not set `PAYMONGO_LIVE_PLATFORM_SPLIT_CONFIRMED=true`, do not advertise automatic DGFY 1% split settlement, and do not run a real-customer PayMongo split-payment checkout until PayMongo explicitly enables the account and provides/validates the parent/platform merchant ID.
...
10. API-generated tenant child merchant IDs are not a substitute for the DGFY parent/platform merchant ID used as the fixed 1% split recipient.
11. Live split checkout must fail closed unless the platform split capability confirmation env flag is explicitly set from external PayMongo evidence.
12. DGFY must not operate as the fallback collector or settlement intermediary while PayMongo Linked Accounts is unavailable; any interim payment flow must avoid custody/redistribution by DGFY unless separate legal, compliance, accounting, and product governance approves a different regulated-money-flow model.
