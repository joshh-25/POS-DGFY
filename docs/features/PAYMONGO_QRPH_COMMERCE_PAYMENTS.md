...
- Sandbox/internal work may continue with API-generated tenant child merchant IDs, but production/live checkout must remain disabled until PayMongo confirms the parent/platform split capability. PayMongo support's June 2026 response did not confirm self-service platform onboarding or provide the parent merchant ID, so this is an external go-live blocker.
...
- Production live customer use remains blocked while PayMongo has not confirmed the parent/platform merchant ID and live split-payment marketplace capability.
