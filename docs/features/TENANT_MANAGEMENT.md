...
- The June 22, 2026 production runtime proof in this workspace is SHA `32c0c5ebd036ab1ffa928647b7cb2ebd65361c52`, recorded by `/var/www/skupervisor/logs/deploy/deploy_20260622_100750.summary.txt` and live `/api/v1/health.services.observability.runtime_sha`. That runtime includes the batch registration handoff behavior: active company creation uses the DGFY account session to create an IMS tenant session and falls back to a visible manual-login path only if tenant-session creation fails. It also includes the founder tenant-user linkage fallback for production raw-insert metadata, unique DGFY session token ids so immediate logout-then-login cannot reuse a blacklisted token, Storefront delivery-map blank-space hardening, canonical side-tracking-drawer cleanup, IMS storefront location map pinning reliability, PH-local reverse-geocode metadata, same-origin `/openfreemap` MapLibre resource routing, and multi-interval Storefront business hours, with live bundle/source inclusion verified after deployment. Controlled production DGFY business-registration UAT passed with evidence `.tmp/production-uat/dgfy-business-handoff/evidence-20260620054511.json`: the run created a QA DGFY account and company, proved `/api/v1/dgfy/auth/tenant-session` returned an IMS tenant token, proved a second tenant-session call was not rate-limited, verified founder membership/ownership linkage, and cleaned up the QA account, tenant, membership, legal acknowledgement rows, and tenant database. Future releases must keep exact QA deployed-head parity deterministic. The June 22 map/hours/tooling deploy used owner-authorized emergency bypass only because pre-deploy SHA parity evidence could not match a target that was not live yet, then closed runtime parity through deploy summary, matching `expected_commit`, live health proof, and deploy-tooling hardening for CRLF-safe SHA arguments, linked-worktree pushes, and QA prereq validation.
