...
- **Mandatory Manual Review**: Public company registration always creates a pending landlord tenant plus a landlord-scoped registration application and immutable attempt. It does not provision a tenant database, activate the company, disclose a company token, or issue a tenant/POS session before Platform Admin approval.
- **Applicant Status Flow**: After submission, both the SKUpervisor and Storefront registration surfaces open `/register-company/status/:applicationId`. The submitting DGFY account can view the review/provisioning state, read rejection reasons, and resubmit corrected details. Pending applicants never receive a `Proceed to POS` action.
...
- **Provisioning Trigger**: Database provisioning runs only after a Platform Admin approves the pending application. A failed provisioning attempt remains auditable and retryable; it must not activate the tenant or leave a reusable partial tenant database.
- **Storefront Handoff**: `Register Your Business` runs inside the signed-in DGFY/Storefront business-registration surface. A successful submission opens the owner-authorized SKUpervisor status route using the returned `application_id`; it does not attempt a tenant-session or POS handoff. Legacy one-time DGFY handoff tokens remain a compatibility path for cross-surface authentication, not an activation bypass.
- **Email Mapping**: Pending registrations create the founder email-to-tenant mapping at registration time so lookup can report pending status. Provisioning completes the active tenant-user and accepted-membership links only after approval succeeds.
- **Abuse Control**: Public company registration is IP rate-limited (`RATE_LIMIT_TENANT_REGISTRATION_MAX_REQUESTS=5` per `RATE_LIMIT_TENANT_REGISTRATION_WINDOW_MS=3600000` by default in production). Keep this strict to protect landlord application creation and the later tenant-provisioning queue.
...
- Every public registration requires a Platform Admin to click **Approve** in Tenant Manager. The approval transition owns tenant-database provisioning and remains the only public-registration path from pending to active.
...
- Once the application status becomes `ready`, the owner may start a normal tenant session from the status page. The tenant master-admin row and accepted founder membership are created during approved provisioning; the registration response itself returns neither tenant auth tokens nor a company-token capability.
...
1. A public company application is submitted, so pending lookup can resolve the applicant; active tenant-user linkage is completed only after approved provisioning succeeds
