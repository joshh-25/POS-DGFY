...
- When a discovery map has at least `clusterMinDataPoints` (default 8) address groups, pins additionally zoom/density-cluster: a MapLibre `cluster: true` GeoJSON source (Supercluster) groups nearby address pins as the map zooms out, rendered as a distinct circular badge (no pin tail, sized by point count) from the pin-shaped same-address badges. Clicking a density cluster zooms the map in to split it apart and scopes the Discover Nearby/View Results panel to its storefronts, same as a same-address cluster click. Below the threshold (including all single-storefront branch-picker widgets), rendering is unchanged from the address-grouping-only behavior above. Implemented in `discoveryMapClustering.js`/`StoresMap.jsx`; see `PLAN/storefront-discovery-map-clustering.md`. Locally implemented and unit-tested (see `discoveryMapClustering.test.js`); not yet confirmed with rendered browser proof against `/map-dgfy` — do that before marking this a `PASS` UAT slice in the log below.
...
- Storefront map pins and shared-coordinate cluster pins render as DOM `maplibregl.Marker` overlays (`StoresMap.jsx`), not MapLibre GeoJSON source/layer symbols — an earlier renderer-owned GeoJSON layer path (`discoveryMapLayers.js`) was built but was superseded by the current DOM-marker implementation and has since been removed as dead code (see the discovery-map clustering plan). Marker cards remain DOM popups anchored to their stored coordinates. The customer "your location" marker renders below storefront pins so overlapping store pins remain clickable.
...
- Storefront map/search stabilization is locally validated as of 2026-06-09. Submitted text searches stay storefront-first, item matches render tenant storefront result cards without auto-opening marker preview cards, repeated identical searches do not refit/flicker, duplicate-coordinate and 13-meter near-coordinate groups render as count pins that scope the existing results panel, hover previews collapse on pin leave, and click/tap previews persist until closed.
- Storefront map pin stability was production-live in SHA `744aa83e652a2847e362b12a8c55e0b18344cc5c` on a renderer-owned GeoJSON discovery layer contract that replaced the DOM `maplibregl.Marker` path at the time. That layer path was later superseded and the map has since reverted to (and, as of this clustering work, continues to use) DOM `maplibregl.Marker` overlays as the live implementation. Storefront discovery pins and count clusters stay anchored to stored IMS coordinates across zoom/pan, marker preview popups remain anchored above normal pins, and count-cluster clicks scope the right-side results panel instead of rendering an on-map list.
- Storefront current-location map controls are production-live in SHA `eed354686109418e8766be97e05bbc90aa8ef55b`. The discovery map exposes a visible `Use current location on map` action, calls the existing browser geolocation-backed Near Me handler only after customer action, requests discovery with `pin_scope=nearest_matching_branch`, updates to `My location` after coordinates are shared, and renders the customer location dot through the MapLibre user-location source/layer below storefront pins.
...
- Production-deployed current-location control coverage: `npx vitest run apps/store/src/__tests__/discoveryFlow.integration.test.jsx -t "exposes a mobile map control"` passed for the mobile map control, `pin_scope=nearest_matching_branch`, and user-location GeoJSON feature coordinates; `npm --prefix frontend exec vitest run apps/store/src/__tests__/discoveryMapLayers.test.js` passed for the layer/source contract. Storefront production build, architecture, and whitespace checks also passed before deploying SHA `eed354686109418e8766be97e05bbc90aa8ef55b`.
...
- Local rendered route health: `PASS` for `/map-dgfy` page identity, desktop search controls, map region, overlay-free render, search-field interaction, and zero relevant console warnings/errors. June 22 Storefront map regression validation passed the layer-model, DOM, marker preview, marker CSS, and `StoresMap` source-contract suites (`5` files / `17` tests), `npm --prefix frontend run build:store`, `npm run check:architecture`, and `git diff --check` for the pin-stability restore. The later current-location control restore added focused Storefront discovery integration and `discoveryMapLayers` coverage, then production route smoke passed for `https://dgfy.ph/map-dgfy` after deploying SHA `eed354686109418e8766be97e05bbc90aa8ef55b`. The latest production evidence available in this workspace is deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260626_161139.summary.txt` for SHA `c375dd90f2db16d8ed924c0b10ecd653319e8b16`, reporting Storefront runtime evidence through public endpoint checks, frontend asset parity passing, live runtime SHA health proof, and deployed Storefront map bundle/source inclusion checks inherited from the Storefront runtime chain. Seeded-data zoom QA remains useful for real-pin placement evidence.
