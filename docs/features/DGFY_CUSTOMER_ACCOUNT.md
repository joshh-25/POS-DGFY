...
- Guest public tracking uses one completion-based request loop per normalized tracking PIN. The visible loop waits five seconds after a completed request before polling again, backs off for terminal or hidden states, honors `Retry-After`/`retryAfterSeconds` on `429`, preserves the last successful status during transient failures, and uses a dedicated read-rate-limit bucket separate from claim/cancel mutations.
