...
The DGFY multi-company switching, DGFY-only company-access hardening, post-merge Storefront customer-flow hardening, updated customer dashboard, June 18 CSRF/POS terminal-lock remediation, iMin receipt preview scroll hotfix, Storefront/POS browser-authenticated request consolidation, DGFY account notification/activity flow, June 19 batch findings remediation, delivery-map blank-space hardening, canonical side-tracking-drawer cleanup, and business-registration IMS handoff hotfix are included in the latest production runtime SHA `13fbea257585187f75b0b3e8f87f391192d9ae97`. Production deploy summary `/var/www/skupervisor/logs/deploy/deploy_20260620_045435.summary.txt` records `deployed_head`, `remote_head`, and `expected_commit` all matching that SHA, and live `/api/v1/health` reports `services.observability.runtime_sha=13fbea257585187f75b0b3e8f87f391192d9ae97` with source `deploy_state:last_deployed_commit`. The deployed contract includes IMS company switching, Storefront account Business / Your Businesses invitation visibility, pending invitation acceptance after `dgfy_business_step_up`, selected-company opening into SKUpervisor, no `company_token` exposure in switcher/invitation/customer surfaces, direct IMS switcher loading only when the tenant-local user is linked to an accepted DGFY membership, founder/master-admin membership bootstrap from verified ownership evidence, current-company rendering even when the accepted membership list contains only the active tenant, return-target allowlist enforcement before handoff-token append, no browser-readable DGFY/customer token persistence, explicit sign-out suppression of immediate cookie-backed customer restoration, no silent same-email tenant-local customer claiming without explicit `dgfy_account_id`, shared tenant/admin and Storefront CSRF/session request helpers, cookie-backed business-registration IMS tenant-session handoff with visible manual-login fallback copy, automatic IMS `auth:login` event emission after DGFY tenant-session exchange, dedicated POS lock-before-open-shift sequencing, scrollable iMin receipt preview controls, account-owned order activity refresh without guest adoption by email or phone, in-app account notifications backed by authenticated APIs plus SSE with polling fallback, deterministic mobile delivery-map sizing, and removal of the duplicate standalone mobile tracking-list card when the side tracker is already available. Local regression coverage for the handoff also asserts that the tenant-session exchange emits the IMS `auth:login` event and isolates browser global stubs between tests. Read-only production checks confirmed the deployed runtime SHA, public Storefront/IMS/POS route reachability, live Storefront bundle inclusion for delivery-map hardening and tracking cleanup, and unauthenticated `401` behavior for protected notification endpoints; controlled production order-mutation, company-registration, and checkout delivery-map UAT is still required before claiming live POS status propagation, live business-registration handoff, or live checkout-map visuals have been proven against production data.
