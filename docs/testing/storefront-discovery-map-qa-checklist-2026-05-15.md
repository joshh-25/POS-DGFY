...
| 2 | Search bar sizing and controls | Frontend | Taller/older search field and actions | Slimmer search bar with explicit `Search` and an icon-only location action titled `Use my current location` | Inspect search bar height and action buttons | **Pass:** search field and button are visibly slimmer, `Search` is visible, and the location icon has the current title. **Fail:** old size/style or visible `Near Me` text remains |
