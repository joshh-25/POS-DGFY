...
9. Tenant-visible platform capability messaging is source-current but not advanced as production-deployed by this checklist. It adds reactive in-app copy for `TENANT_CAPABILITY_DISABLED` and `CUSTOMER_ACCESS_MODE_BLOCKED`, tenant-wide IMS/POS disabled notices, Storefront access-mode blocked-action copy, and Tenant Manager capability-change impact previews.
