...
| DGFY POS unlock and terminal pairing | Accepted DGFY account or grace-eligible legacy tenant user | DGFY session then POS tenant session, or tenant-local credentials during legacy grace | Accepted membership, active tenant, POS permission, active registered terminal, authorized terminal location, and correct terminal password; legacy fallback must resolve tenant through `/auth/lookup` and validate tenant credentials | Company selected before terminal; terminal password is independent and write-only; signed HttpOnly pairing is bound to tenant, terminal, location, authorization profile, membership, and password fingerprint; bindings are revalidated on paired use | `pos_unlock_attempted`, `pos_unlock_success`, `pos_unlock_failed`; pairing denial uses POS route/audit telemetry; legacy audit wiring remains tracked separately | No membership, no POS permission, inactive terminal, wrong password, changed password, location denial, failed lookup, or invalid tenant credentials blocked | POS drawer DGFY sign-in, company select, terminal select/password, invalidation cases, admin no-shift navigation, cashier restrictions |
