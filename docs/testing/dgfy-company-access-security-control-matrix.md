...
| DGFY POS unlock | Accepted DGFY account or grace-eligible legacy tenant user | DGFY session then POS tenant session, or tenant-local credentials during legacy grace | Accepted membership, active tenant, POS permission, terminal registry/location policy; paired devices also require valid signed pairing fingerprint and cashier location grant; legacy fallback must resolve tenant through `/auth/lookup` and validate tenant credentials | First use requires dedicated terminal unlock/password; a valid ADR 0030 device pairing may go directly from cashier login to Opening Cash; dedicated POS stays locked until pairing or explicit unlock succeeds; development tenant auto-login is opt-in and excluded from terminal lock routes | `pos_unlock_attempted`, `pos_unlock_success`, `pos_unlock_failed`; legacy audit wiring remains tracked separately | No membership, no POS permission, inactive tenant, terminal denial, invalid/changed pairing, location-grant mismatch, failed lookup, or invalid tenant credentials blocked | POS drawer login, one-time terminal pairing fallback, paired Opening Cash flow, legacy fallback, open-shift prompt only after device authorization |
