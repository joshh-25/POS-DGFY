...
| DGFY POS unlock and logical terminal use | Accepted DGFY account or grace-eligible legacy tenant user | DGFY session then POS tenant session, or tenant-local credentials during legacy grace | Accepted membership, active tenant, POS permission, active registered terminal, and authorized terminal location; invited cashiers must be scoped to the selected company and to terminal locations granted to their tenant-local authorization profile; legacy fallback must resolve tenant through `/auth/lookup` and validate tenant credentials | Company selected before terminal; selected terminal must be active and location-bound; non-admin DGFY cashiers must not depend on admin-only company-info or user-list setup endpoints before unlock; operator identity and location grants are revalidated on terminal use; physical-device pairing is optional compatibility state only | `pos_unlock_attempted`, `pos_unlock_success`, `pos_unlock_failed`; terminal and location denials use POS route/audit telemetry; legacy audit wiring remains tracked separately | No membership, no POS permission, inactive terminal, missing terminal location, unauthorized location, failed lookup, or invalid tenant credentials blocked | POS drawer DGFY sign-in, company select, logical terminal selection, admin no-shift navigation, cashier restrictions |
