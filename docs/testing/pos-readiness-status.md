...
3. Fixed root causes and latest verification:
   ...
   - `POSCheckoutTerminal.jsx`, `SkupervisorPOSCheckoutTerminal.jsx`, and `ReceiptPrintView.jsx` now share `checkoutSurfaceContract.js` for the governed DGFY fee contract and checkout payload shape. The new contract coverage proves terminal ID propagation, payment handoff mode, discount metadata, F&B check/table/server/guest fields, line modifiers, kitchen station override, scan metadata, and receipt fallback label behavior.
