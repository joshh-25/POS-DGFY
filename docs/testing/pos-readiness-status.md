...
3. Fixed root causes and latest verification:
   ...
   - `npm run smoke:pos-terminal-ui` now provides repeatable desktop/tablet/mobile locked-state smoke and standalone POS `/sales` handoff evidence. The redirect proof accepts a failed request to `http://localhost:5173/sales?...` when the SKUpervisor dev server is offline, because that still proves standalone POS emitted the intended handoff URL.
