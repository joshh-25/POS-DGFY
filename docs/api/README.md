...
4. Company user invitations are DGFY-account-first. IMS admins search registered active DGFY accounts, invite a selected account, and the invited user accepts or rejects from DGFY My Account -> Business.
5. Admin user-management invitation actions are documented in `docs/api/specification.md`: DGFY account search, create DGFY invitation, include pending/declined invitations, and cancel pending invitations. New direct invite links, resend-link, and manual-link recovery are retired by default.
