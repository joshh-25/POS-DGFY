...
- `GET /api/v1/geo/reverse-geocode` returns first-party PH-local address metadata for IMS map suggestions. Successful responses include `provider="dgfy-ph-local"`, `precision` (`barangay`, `city`, `province`, or `coordinate_only`), and optional `distance_meters`. The endpoint must not return vague `Near ...` labels; when no local match is available it returns `Pinned location (lat, lon)` with `precision="coordinate_only"`.
