...
**Rate Limit Contract**: Public reads use a dedicated IP + normalized tracking-PIN bucket sized for five-second completion-based polling. Claim and cancellation mutations remain on the stricter Store tracking mutation limiter. `429` responses include `Retry-After` and `retryAfterSeconds`; clients must retain the last successful status and delay the next request for at least that duration.
