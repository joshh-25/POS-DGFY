...
| `EMAIL_DELIVERY_PROVIDER=brevo_api` + real `BREVO_API_KEY` in beta's `.env` | No (historical, see note) | Plain server-side `.env` file, not tracked in git. At the time of this incident a fresh server needed these set manually because Linode blocks outbound SMTP by default and Brevo's HTTPS API was the working path. **Note (2026-08-07, issue #279): the Brevo HTTPS API fallback has since been removed entirely** — Brevo was DMARC-blocked for Yahoo/iCloud recipients regardless (#135), and production now delivers via Namecheap SMTP. A fresh server today needs `SMTP_*`/`EMAIL_FROM` set manually instead; there is no HTTPS-only fallback path if outbound SMTP is blocked on a new host — confirm SMTP egress works before provisioning. |
