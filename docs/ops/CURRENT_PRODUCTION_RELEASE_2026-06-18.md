...
- Production remote `HEAD`: `27e5ce1249136ef8676392fec5b6e2c027d459e5`
- Production `.deploy-state/last_deployed_commit`: `27e5ce1249136ef8676392fec5b6e2c027d459e5`
- Latest production deploy summary path: `/var/www/skupervisor/logs/deploy/deploy_20260618_194442.summary.txt`
- Deploy summary `deployed_head`, `remote_head`, and `expected_commit`: `27e5ce1249136ef8676392fec5b6e2c027d459e5`
- Live production `/api/v1/health` reports `services.observability.runtime_sha=27e5ce1249136ef8676392fec5b6e2c027d459e5` with source `deploy_state:last_deployed_commit`.
...
- Artifact: `.tmp/release-gates/27e5ce1249136ef8676392fec5b6e2c027d459e5/release_verdict.json`
- Verdict: `bypassed`
- Failed gates: `1`
- Failed gate: stale QA deploy-summary SHA parity before production promotion (`deployed_head=41b988b021b7b14cf46da3b93aada66a2688785f`; `target_sha=27e5ce1249136ef8676392fec5b6e2c027d459e5`)
...
- Post-deploy proof closes the runtime parity risk for this deployed SHA: production deploy summary, remote head, expected commit, live `/api/v1/health.services.observability.runtime_sha`, and frontend asset parity all match `27e5ce1249136ef8676392fec5b6e2c027d459e5`.

Observability evidence:

- Artifact: `.tmp/release-gates/27e5ce1249136ef8676392fec5b6e2c027d459e5/observability_evidence.json`
...
The current production runtime includes the branch and PR adoption chain from the earlier proven production SHA through `27e5ce12`:
...
- PR #20 POS and DGFY Storefront closeout: POS safety and DGFY Storefront customer-flow updates are in `master` and included in the deployed `27e5ce12` runtime chain.
...
## Source-Current But Not Yet Production-Proven

The local source tree also contains Storefront/POS browser-authenticated request consolidation that routes Storefront hospitality booking through the shared Storefront `requestJson` helper and moves IMS/POS dev auto-login to the shared tenant login/session service. Source-current DGFY account notification and order activity plumbing refreshes explicit account-owned order snapshots and notification events without guest adoption by email or phone. These slices have local source-current test evidence, but they are not included in the production `27e5ce12` runtime until a later guarded deploy records a new runtime SHA.
