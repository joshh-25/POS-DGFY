...
- `auto_standard` immediately provisions non-subscription registrations from a signed-in DGFY account and then the frontend exchanges the accepted founder membership for a normal tenant session, with manual login fallback if tenant-session exchange fails.
