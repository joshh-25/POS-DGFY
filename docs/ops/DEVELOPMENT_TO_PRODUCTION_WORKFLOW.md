...
The inventory records included and excluded files, risk, affected surfaces, tests, docs, ADR/compliance status, commit boundary, rollback notes, production proof, independence, and verdict.
...
Required gates are docs lint, architecture checks, compliance checks, dependency audits, production env fixtures, backend matrix, frontend tests, all frontend builds, frontend budgets, development-to-production script tests, merge-adoption-required, deploy source contract, and batch inventory completeness.
...
The CI wrapper requires `RELEASE_TARGET_SHA`, fetches `origin/master`, requires equality, refuses dirty state, never pushes/merges/rebases/prompts, avoids interactive SSH TTY allocation, runs deploy source and no-staging gates, then SSHes to production and runs:
...
Allowed accuracy states are `accurately reflected`, `partially reflected`, `source-current only`, `deployed but behavior not proven`, `docs overclaim`, and `deployed but inaccurate`.
