...
**Critical caveat that applies to every "(a) Covered" row below**: `promotion-quality-gate.yml`
(the CI workflow all "(a)" rows point at) is **entirely advisory** as of #1063/#1066/#1253 — every
step in every quality job carries its own `continue-on-error: true`, so a red run never blocks a
promotion PR's `mergeStateStatus` today. "Covered" here means *the same command runs in CI and its
result is visible* (via the `report-advisory-failures` job's tracking-issue comment on
`#1124`), not *CI enforces this gate*. Per #1147's own "Must hold" bullet, a gate only moves from
"local is authoritative" to "CI replaces local" once its CI job has been verified trustworthy
against #1124's audit standard — that verification hasn't happened for any gate yet, "(a) Covered"
or not.
...
| 4 | `docs.lint` | `node scripts/lint-docs.js && npm run check:adr` (via `npm run lint:docs`) | **(a)** | `repository-quality`'s "Run governed documentation lint" step runs the identical `npm run lint:docs`. |
| 5 | `architecture.guardrails` | `cd apps/dgfy-api && npm run check:architecture-guardrails && npm run check:controller-boundaries` (via `npm run check:architecture`) | **(a)** | `dgfy-api-quality`'s "Enforce modular architecture guardrails" + "Enforce controller architecture boundaries" steps run the identical two subcommands. |
...
| 9 | `backend.lint` | `npm --prefix apps/dgfy-api run lint` | **(a)** | `dgfy-api-quality`'s "Run API lint" step, identical command. |
| 10 | `backend.test_matrix` | `node scripts/run-backend-test-matrix.js` (via `npm run test:backend:matrix`) | **(a)**† | `dgfy-api-quality`'s "Run dgfy-api test matrix" step runs the identical command. † This is the gate #1147's own body calls the "hard technical prerequisite": it already runs in CI today, advisory, at ~20-24min per the #1015 baseline measurement — moving it from advisory to blocking before #1015 lands would reproduce #345's per-run cost problem in a new place, which is exactly what #1147/#1018 are structured to avoid. |
| 11 | `frontend.ims.lint` | `npm --prefix apps/dgfy-ims run lint` | **(a)** | `frontend-ims-quality`'s "Run dgfy-ims lint" step, identical command. |
| 12 | `frontend.pos.lint` | `npm --prefix apps/dgfy-pos run lint` | **(a)** | `frontend-pos-quality`'s "Run dgfy-pos lint" step, identical command. |
| 13 | `frontend.storefront.lint` | `npm --prefix apps/dgfy-storefront run lint` | **(a)** | `frontend-storefront-quality`'s "Run dgfy-storefront lint" step, identical command. |
...
| 15 | `frontend.storefront.contracts` | `cd apps/dgfy-storefront && npx vitest run contract.test integration.test` (via `npm run test:frontend:contracts:storefront`) | **(a)**† | `frontend-storefront-quality`'s "Run storefront vitest suite" step runs unfiltered `npx vitest run`. Storefront's `vite.config.js` `test.exclude` only excludes `tests/e2e/**` (Playwright's own domain) — every other vitest file runs, which is a strict superset of the local gate's 26-file `contract.test`/`integration.test` pattern. † Superset, not identical command — flagged so a future edit to either side doesn't assume byte-for-byte parity. |
