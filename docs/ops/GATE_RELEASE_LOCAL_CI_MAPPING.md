...
| 10 | `backend.test_matrix` | `node scripts/run-backend-test-matrix.js` (via `npm run test:backend:matrix`) | **(a)**† — **still advisory, deliberately not flipped by #1431 Phase 1** | `dgfy-api-quality`'s "Run dgfy-api test matrix" step runs the identical command. † This is the gate #1147's own body calls the "hard technical prerequisite": it already runs in CI today, advisory, at ~20-24min per the #1015 baseline measurement — moving it from advisory to blocking before #1015 lands would reproduce #345's per-run cost problem in a new place, which is exactly what #1147/#1018 are structured to avoid. **Root cause confirmed 2026-09-02 (#1431 Phase 1 plan, §1b):** not just slow — genuinely failing for real, both a hosted-runner OOM (~3.5GB heap, V8 `Ineffective mark-compacts` kill on `ubuntu-latest`'s fast tier) and real, deterministic test/fixture/snapshot rot in 6 files (stale ESM mock factories, a stale seeded-industry count, a stale `LOCK.UPDATE` mock, a snapshot missing a `laundry` entry — reproduced locally on `develop` `f3c8f43fd`). A `tenantSchemaBootstrap.integration.test.js` db-tier timeout is a fourth, separately unconfirmed issue. Flipping this gate needs a dedicated fix PR (6 stale files + `CI=true` on the fast tier + a heap/runner decision tied to #1365/#1015) before #1015 itself, not after — tracked as a hand-back follow-up from #1431 Phase 1. |
