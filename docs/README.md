...
39. The front-facing DGFY customer account surface is landlord-scoped and cross-tenant. DGFY customer dashboard/activity/history/recovery reads from landlord activity tables, while a bounded request-time sync backfills recent tenant order activity only for latency safety. Full historical completeness uses `npm run backfill:dgfy-customer-activity:apply`, which runs landlord migrations first and then scans POS orders, F&B checks linked through POS transactions, Services bookings, and Hospitality reservations. Production dry-runs can require mode evidence with `--require-activity-types=order,service_booking,hospitality_booking,fnb_order`.
40. Fulfilled guest item reviews are an additive Storefront tracking path. Completed-order tracking can return single-use `review_invites`; guest token validation/submission creates pending moderated reviews without linking or mutating signed-in DGFY account history. Signed-in review submission remains account-gated and purchase-gated by ADR 0023.
