...
42. The DGFY POS surface split is a local release candidate after the May 30, 2026 cleanup. Standalone POS owns cashier terminal routing and SKUpervisor `/pos` owns admin/mode panels and Sales/reporting handoff; POS lint, frontend lint errors, backend lint, production builds, frontend budgets, compliance, architecture, `npm run smoke:pos-terminal-ui`, local desktop/mobile/tablet locked-state smoke, Sales redirect proof, shared checkout payload/fee contract tests, and `npm run gate:release:local` now pass for merge SHA `6ca61e8d01e304a75f078d11f937aec4425c944e`. POS onboarding initial chunk bloat was reduced by lazy-loading the map picker. Production promotion still requires human cashier/admin UAT and exact deployed-SHA verification.
