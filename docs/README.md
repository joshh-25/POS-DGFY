...
33. Third-party Storefront map integrations such as MapViu use the public auto-parser feed `GET /api/v1/storefront/discovery/map-pins` and the path-limited `PUBLIC_API_CORS_ORIGIN` allowlist. The discussion and deployment notes are recorded in `docs/features/STOREFRONT_MAPVIU_PUBLIC_API_2026-05-29.md`.
34. Storefront mode presentation is template-driven. Services Mode uses service-specific view-model grouping, availability/hold-backed booking drafts, batch booking, and per-booking confirmation/payment rendering; F&B uses restaurant/menu grouping, default modifier selection, allergen presentation, and a reservation entry point; simple/MSME keeps a lightweight product storefront. The May 14, 2026 local production candidate passed 74 storefront app tests and a tenant-store build with a fresh vendor chunk, replacing the previously reported failing `vendor-BGNbcnYt.js` runtime path.
35. Account-phone rollout is now explicit and staged. New company/user registration and invite acceptance require phone numbers, existing users can correct numbers in Settings > Profile, admins can isolate missing-phone accounts in User Management, and backend enforcement defaults to `PHONE_COMPLETION_ENFORCEMENT_MODE=observe` outside tests. Operators can pilot clean tenants through `tenant_allowlist`, inspect unresolved users with `npm --prefix backend run verify:phone-rollout:users`, prove current config safety with `npm --prefix backend run verify:phone-rollout:config-safe`, and only switch to global `all` mode after `npm --prefix backend run verify:phone-rollout:complete` passes.
36. Account passwords require only a minimum of 8 characters across tenant user registration, invitation acceptance, company founder registration, and authenticated Settings > Profile password changes. The frontend password generator is optional convenience UI, defaults to a readable 16-character password, and fills matching confirmation fields where present.
37. Frontend release hardening includes a governed route-chunk budget gate. `npm run check:frontend-budgets` enforces login, POS, terminal, and sales route ceilings, treats lazy `vendor-maplibre-*` as a separately capped map dependency, and still reports unrelated vendor growth as actionable.
38. Compliance-sensitive delivery work now has two paired branch gates: `npm run check:compliance` requires a declaration file for sensitive paths and enforces computed minimum classification plus strict `major|regulatory` preflight metadata, while the bundled API-contract check fails when compliance-sensitive runtime fields drift from `docs/api/specification.md`.
39. Dependency audits are release gates. `npm run audit:dependencies:prod` and `npm run audit:dependencies` cover root, backend, and frontend package scopes, run in CI, and are included in `npm run gate:release:local`. The June 2, 2026 audit package records dependency vulnerabilities as resolved with zero current npm advisories in locked dependency trees.
40. No-staging production promotion requires exact QA deploy parity. `qa_deploy_summary.txt` must show the same `deployed_head` as `RELEASE_TARGET_SHA`; stale QA evidence is a hard release failure unless an explicit emergency bypass is recorded.
41. The front-facing DGFY customer account surface is landlord-scoped and cross-tenant. DGFY customer dashboard/activity/history/recovery reads from landlord activity tables, while a bounded request-time sync backfills recent tenant order activity only for latency safety. Full historical completeness uses `npm run backfill:dgfy-customer-activity:apply`, which runs landlord migrations first and then scans POS orders, F&B checks linked through POS transactions, Services bookings, and Hospitality reservations. Production dry-runs can require mode evidence with `--require-activity-types=order,service_booking,hospitality_booking,fnb_order`.
42. Fulfilled guest item reviews are an additive Storefront tracking path. Completed-order tracking can return single-use `review_invites`; guest token validation/submission creates pending moderated reviews without linking or mutating signed-in account history. Signed-in review submission remains account-gated and purchase-gated by ADR 0023.
43. The DGFY POS surface split is production-deployed with human UAT pending after the May 31, 2026 cleanup. Standalone POS owns cashier terminal routing and SKUpervisor `/pos` owns admin/mode panels and Sales/reporting handoff; the production deployment evidence recorded POS lint, frontend lint errors, backend lint, production builds, frontend budgets, compliance, architecture, `npm run smoke:pos-terminal-ui`, local and production desktop/mobile/tablet locked-state smoke, Sales handoff proof, shared checkout payload/fee contract tests, `npm run gate:release:local`, production health, public endpoint checks, frontend asset parity, and exact deployed-head verification as passing for that release. The June 2, 2026 system audit records new current local-release blockers for backend lint and POS route chunk budgets that must be closed before the next production release. POS onboarding initial chunk bloat was reduced by lazy-loading the map picker. Final `9.2+` readiness still requires human cashier/admin UAT for authenticated checkout, receipt, barcode, Sync Queue, and mode-specific panel flows.
44. POS receipt fiscal/non-fiscal rendering is server-contract driven. Print surfaces use explicit `receipt_contract` or persisted transaction `document_type` plus `document_context`; invoice number prefixes such as `INV-` and `NFS-` are sequence identifiers only and must not be used to infer fiscal status.
