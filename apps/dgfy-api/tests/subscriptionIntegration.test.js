...
describe('Subscription integration hard-disable policy', () => {
    ...
    it('requires DGFY authentication before subscription registration reaches tenant registration policy', async () => {
        const response = await request(app)
        ...
        expect(response.body?.success).toBe(false);
        expect(response.body?.message).toBe('DGFY account authentication is required.');
    });
});
