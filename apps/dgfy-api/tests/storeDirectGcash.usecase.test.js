...
describe('Storefront direct wallet payment-session routing', () => {
  ...
  it('creates a card Payment Intent without creating Hosted Checkout', async () => {
    const createdSession = {
      session_id: 18,
      public_reference: 'CPS-DIRECTCARD',
      status: 'created',
      provider: 'paymongo',
      checkout_payload: JSON.stringify({ payment_type: 'card' }),
      total_amount: 90,
      total_amount_centavos: 9000,
      currency: 'PHP',
      platform_fee_centavos: 90
    };
    const updatedSession = {
      ...createdSession,
      status: 'awaiting_payment',
      provider_payment_intent_id: 'pi_live_card',
      provider_payload: {
        ...
        paymentIntent: {
          id: 'pi_live_card',
          attributes: { client_key: 'pi_live_card_client_key' }
        }
      },
      expires_at: new Date(Date.now() + 60 * 60 * 1000)
    };
    ...
    const paymongoService = {
      createDirectCardPaymentIntent: jest.fn().mockResolvedValue({
        paymentFlow: 'direct_card',
        publicKey: 'pk_live_fixture',
        returnUrl: 'https://dgfy.ph/tenant-store/masu-cafe/order?payment_status=return',
        paymentIntent: {
    ...
    const useCase = buildStoreCheckoutPaymentSessionUseCase({
      ...
      commerceQrphEnabled: false,
      commercePaymongoSplitEnabled: false,
      directCardEnabled: true,
      directCardRequested: true,
      requireCommercePaymentConfig: jest.fn().mockReturnValue([])
    });
    ...
    const result = await dbStore.run({ tenantId: TENANT_ID, tenantToken: 'masu-cafe' }, () => useCase({
      payload: {
        store_slug: 'masu-cafe',
        payment_type: 'card',
        idempotency_key: 'direct-card-1234',
        customer_name: 'Test Customer',
        customer_email: 'customer@example.com',
        customer_phone: '+639171234567',
        order_method: 'pickup',
    ...
    expect(result.data.payment_session).toEqual(expect.objectContaining({
      ...
      provider_payment_intent_id: 'pi_live_card',
      paymongo_public_key: 'pk_live_fixture',
      paymongo_client_key: 'pi_live_card_client_key',
      checkout_url: null
    }));
    expect(paymongoService.createDirectCardPaymentIntent).toHaveBeenCalledWith(expect.objectContaining({
      amount: 9000,
      currency: 'PHP',
      returnUrl: expect.stringContaining('payment_status=return')
    }));
  ...
  it('fails closed instead of falling back to Hosted Checkout when direct payment is required', async () => {
    const commercePaymentRepository = {
      findSessionByIdempotency: jest.fn().mockResolvedValue(null),
    ...
    const result = await dbStore.run({ tenantId: TENANT_ID, tenantToken: 'masu-cafe' }, () => useCase({
      payload: {
        store_slug: 'masu-cafe',
        payment_type: 'gcash',
        idempotency_key: 'direct-required-1234',
        customer_name: 'Test Customer',
        customer_email: 'customer@example.com',
        customer_phone: '+639171234567',
        order_method: 'pickup',
    ...
    expect(result.error.details).toEqual(expect.objectContaining({
      code: 'DIRECT_PAYMENT_CONFIGURATION_INCOMPLETE',
      payment_type: 'gcash'
    }));
    expect(commercePaymentRepository.createSession).not.toHaveBeenCalled();
    expect(paymongoService.createDirectGcashPaymentIntent).not.toHaveBeenCalled();
    expect(paymongoService.createHostedCheckoutSession).not.toHaveBeenCalled();
  });
});
