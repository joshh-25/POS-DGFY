// policyEngine.js — port of the read-only legacy
// backend/src/modules/compliance/policy/compliancePolicyEngine.js's
// evaluateComplianceDecision (and its full checklist/receipt-contract/
// preflight support) with EXACTLY ONE deliberate deviation (D-05, the most
// important decision in Phase 8 — see 08-CONTEXT.md before touching this
// file).
//
// D-05 — what changed and why:
// Legacy's compliant_active branch (~compliancePolicyEngine.js lines
// 903-918) denied any POS/receipt operation whose requestedDocumentContext
// was not 'fiscal', forcing every fully-activated business into Fiscal-only
// output (a dedicated "document context not allowed" deny reason code).
// That is wrong for this
// architecture: a formal/BIR-registered business runs two app builds side by
// side (Omni default, Fiscal on explicit customer request for an Official
// Receipt) against the SAME backend — compliant_active must mean "Fiscal is
// now available to choose", not "Fiscal is now mandatory". That deny branch
// has been removed entirely below (search this file for "D-05" to find the
// exact spot); everything else in evaluateComplianceDecision — including the
// non_compliant_active and compliant_pending branches (already correct: they
// deny/require-setup fiscal while always allowing non-fiscal) and the
// compliant_active settings-update + checklist-completeness branches that
// follow — ports unchanged.
//
// This file is a pure-function module: no I/O, no model imports. The
// tenant-scoped state (ComplianceModeState row) is loaded by
// ../repositories/complianceModeStateRepository.js and assembled into the
// `tenant` context object by ../usecases/complianceGate.js before calling
// evaluateComplianceDecision here — this module never touches persistence.
...
import {
    ...
    DOCUMENT_CONTEXTS,
    POS_OPERATIONS as POS_OPERATIONS_LIST
} from './constants.js';
...
const POS_OPERATIONS = new Set(POS_OPERATIONS_LIST);
...
export const evaluateComplianceDecision = ({
    ...
    if (modeState === COMPLIANCE_MODE_STATE.COMPLIANT_ACTIVE) {
        // D-05 DEVIATION (the single deliberate change from legacy): the
        // legacy branch that lived here denied any POS/receipt operation
        // whose requestedDocumentContext wasn't 'fiscal' once a business was
        // compliant_active (a dedicated "document context not allowed" deny
        // reason code), enforcing Fiscal-only output. That branch is INTENTIONALLY OMITTED
        // — compliant_active now allows BOTH 'fiscal' and 'non_fiscal'
        // requestedDocumentContext values (Omni-default, Fiscal-on-request;
        // see 08-CONTEXT.md D-05). Do NOT re-add a deny branch keyed on
        // requestedDocumentContext here.
