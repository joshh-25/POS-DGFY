...
const applyBenefitConfig = (merged) => {
    ...
    // fixed_price legitimately allows 0 (a giveaway); the other two need a positive amount.
    const invalid = missing || (benefitClass === 'fixed_price' ? numeric < 0 : numeric <= 0);
    if (invalid) {
        voucherError(
...
const assertFixedPriceHasScope = (benefitClass, scopes) => {
    if (benefitClass !== 'fixed_price') return;
    if (Array.isArray(scopes) && scopes.length > 0) return;
    voucherError(
        'fixed_price vouchers require at least one item or item folder scope.',
...
export const buildCreateVoucherUseCase = ({ repository }) => async ({
    ...
    try {
        ...
        assertFixedPriceHasScope(values.benefit_class, scopes);
        ...
        // Scope references are validated INSIDE the transaction and BEFORE the voucher_scopes write,
...
export const buildUpdateVoucherUseCase = ({ repository }) => async ({
    ...
    try {
        ...
        assertFixedPriceHasScope(merged.benefit_class, nextScopes);
...
const assertActivationAllowed = ({ voucher, scopes, now, timezone }) => {
    ...
    assertFixedPriceHasScope(voucher.benefit_class, scopes);
