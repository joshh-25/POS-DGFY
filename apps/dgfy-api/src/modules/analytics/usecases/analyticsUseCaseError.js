...
export const mapAnalyticsUseCaseError = (error, fallbackMessage) => {
  if (isDomainError(error)) {
    return error;
  }

  const message = error?.message || fallbackMessage || 'Analytics request failed';
  const statusCode = Number.isInteger(error?.statusCode) ? error.statusCode : undefined;
  const codeFromStatus = statusCodeToDomainErrorCode(statusCode || 500);
  const hasNotFoundMessage = /not found/i.test(message);

  return new DomainError(
    hasNotFoundMessage ? DomainErrorCode.RESOURCE_NOT_FOUND : codeFromStatus,
    message,
    {
