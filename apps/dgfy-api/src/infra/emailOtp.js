...
// Mirrors backend/src/services/emailOtpService.js's hashing, TTL, and
// attempt-limit rules exactly (same hash secret precedence, same 6-digit
// code format, same single-active-OTP-per-scope invariant) so a token
// requested/verified through one service behaves identically to the other —
// but scoped to only the two DGFY purposes this service issues. See
// apps/dgfy-api/src/modules/dgfyAuth/README.md for why this is duplicated
...
const isEmailOtpEnforcementEnabled = () => {
    if (process.env.EMAIL_OTP_ENFORCEMENT_ENABLED === 'true') return true;
    if (process.env.EMAIL_OTP_ENFORCEMENT_ENABLED === 'false') return false;
...
export const requestEmailOtp = async ({ purpose, email, tenantId = null, metadata = {}, emailSender = emailService }) => {
    const { EmailOtp } = registerModels();
    ...
    const codeHash = hashOtpCode({ purpose: normalizedPurpose, email: normalizedEmail, tenantId: normalizedTenantId, code });

    await EmailOtp.update(
        { consumed_at: new Date() },
        { where: { purpose: normalizedPurpose, email: normalizedEmail, tenant_id: normalizedTenantId, consumed_at: null } }
    );
...
export const verifyEmailOtp = async ({ purpose, email, code, tenantId = null, consume = true }) => {
    const { EmailOtp } = registerModels();
    const normalizedPurpose = String(purpose || '').trim();
    const normalizedEmail = normalizeOtpEmail(email);
    const normalizedTenantId = normalizeTenantId(tenantId);
    ...
    const expectedHash = hashOtpCode({ purpose: normalizedPurpose, email: normalizedEmail, tenantId: normalizedTenantId, code: normalizedCode });
    ...
    if (consume) {
        const [updatedCount] = await EmailOtp.update(
            { consumed_at: new Date() },
            { where: { otp_id: otp.otp_id, consumed_at: null } }
        );
        if (updatedCount !== 1) {
            throw createError('Email verification code is missing or expired', 422, 'EMAIL_OTP_EXPIRED');
        }
...
export default { EMAIL_OTP_PURPOSES, requestEmailOtp, verifyEmailOtp };
