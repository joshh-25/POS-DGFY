...
export function DiscoveryHeroMapStage({
  ...
  // map.getCanvasContainer() and only accepts a touch whose original DOM target is
  // a descendant of that container (maplibre-gl/src/ui/handler_manager.ts,
  // `_getMapTouches`: `this._el.contains(target)`). A CSS touch-action rule on a
  // sibling overlay only suppresses the browser's own default page-zoom gesture --
  // it can't retarget a touch that already started on a sibling into the map,
  // because a DOM touch event keeps the element it started on for the whole
  // gesture and never bubbles sideways. StoresMap reports that container here so
  // the interactive overlays below (the ones a two-finger pinch can actually
  // start on) can be portaled into it instead of staying plain siblings.
    ...
    <DiscoveryMapCard viewportMode={discoveryViewportMode}>
      ...
      >
        <StoresMap
          ...
          onCanvasContainerReady={setCanvasContainerEl}
          onSelectStore={(pin) => {
        ...
        {/* #475 RF-1: this bar and the mobile button stack below are the only overlays
            here with real (pointer-events: auto) interactive surface a two-finger pinch
            can land on -- the decorative gradient above has pointer-events: none, so a
            touch on it already passes through to the map underneath and was never the
            problem. Portal both into canvasContainerEl (see the state comment above) so
            a pinch starting on them is treated as a map touch, not a page touch. */}
        {canvasContainerEl && createPortal(
          <div
            style={{
              position: 'absolute',
        ...
        {isDiscoveryMobileViewport && canvasContainerEl && createPortal(
          <div style={{ position: 'absolute', right: 10, bottom: 12, zIndex: 12, display: 'grid', gap: 10, justifyItems: 'end', touchAction: 'none' }}>
