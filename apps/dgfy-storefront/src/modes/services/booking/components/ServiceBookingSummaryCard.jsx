  ...
  return (
    <aside style={{ display: 'grid', gap: 16, position: isMobileViewport ? 'static' : 'sticky', top: 32, minWidth: 0 }}>
      <section style={{ border: '1px solid #cce8ee', borderRadius: 16, background: '#fff', padding: isMobileViewport ? 18 : 24, display: 'grid', gap: 0, boxSizing: 'border-box' }}>
        ...
        <div style={{ fontSize: isMobileViewport ? 30 : 35.2, lineHeight: isMobileViewport ? 1.25 : 1.6, fontWeight: 800, color: '#101010', fontFamily: servicesDisplayFont, marginBottom: 24, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          ...
          <>
            <div style={{ display: 'grid', gap: 7.2, borderBottom: '1px solid #cce8ee', paddingBottom: 24, marginBottom: 24 }}>
              {infoRows.map((row) => (
                <div key={row.label} style={{ display: 'grid', gridTemplateColumns: isMobileViewport ? 'minmax(0, 0.85fr) minmax(0, 1.15fr)' : SUMMARY_TWO_COLUMN_GRID, alignItems: 'center', columnGap: 16, minHeight: 24.3, minWidth: 0 }}>
                  ...
                  <strong style={{ fontSize: 14.4, lineHeight: 1.6, color: '#153e4a', textAlign: 'right', fontWeight: 700, justifySelf: 'stretch', minWidth: 0, overflowWrap: 'anywhere' }}>{row.value}</strong>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'grid', gap: 0 }}>
              ...
              <div style={{ display: 'grid', gap: 2.4, marginTop: 8 }}>
                <div style={{ display: 'grid', gridTemplateColumns: SUMMARY_TWO_COLUMN_GRID, alignItems: 'center', columnGap: 16, padding: '8.8px 0', color: '#58717a', fontSize: 14.4, lineHeight: 1 }}>
                  <span>Subtotal</span><span>{money(bookingSummaryAmount)}</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: SUMMARY_TWO_COLUMN_GRID, alignItems: 'baseline', columnGap: 16, borderTop: '1px solid #cce8ee', marginTop: 12, padding: '16px 0', color: '#101010', fontSize: 18.4, lineHeight: 1.6, fontWeight: 700, fontFamily: servicesDisplayFont }}>
