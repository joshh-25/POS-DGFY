      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobileViewport ? '1fr' : 'minmax(0, 1fr) 370px', gap: 24, marginTop: 28, alignItems: 'start' }}>
        <div style={{ display: 'grid', gap: 24, minWidth: 0 }}>
          <section style={{ background: soft, border: `1px solid ${cardBorder}`, borderRadius: 20, padding: isMobileViewport ? 20 : 30, display: 'flex', justifyContent: 'space-between', alignItems: 'center', overflow: 'hidden', position: 'relative', flexWrap: 'wrap', gap: 20, boxSizing: 'border-box', minWidth: 0 }}>
            ...
            <div aria-hidden="true" style={{ zIndex: 1, display: isMobileViewport ? 'none' : 'grid', placeItems: 'center', minWidth: 100, opacity: 0.85, color: primary }}>{isLocalQuote ? <FileText size={94} strokeWidth={1.4} /> : (isLocalDropoff ? <Package size={94} strokeWidth={1.4} /> : (isPickup ? <ShoppingBag size={94} strokeWidth={1.4} /> : <Truck size={94} strokeWidth={1.4} />))}</div>
          </section>

          <section aria-label="Service status timeline" style={{ position: 'relative', padding: isMobileViewport ? '12px 0' : '10px 0', display: 'flex', justifyContent: 'space-between', overflowX: 'hidden', minWidth: 0 }}>
            ...
            {view.timeline.map((step) => {
              ...
              return <div key={step.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: '1 1 0%', width: isMobileViewport ? 'auto' : 100, minWidth: 0, gap: isMobileViewport ? 6 : 8, zIndex: 1, textAlign: 'center' }}><div style={{ width: 30, height: 30, borderRadius: '50%', background: done || active ? primary : '#fff', border: `2px solid ${done || active ? primary : '#cbd5e1'}`, color: done || active ? '#fff' : '#cbd5e1', display: 'grid', placeItems: 'center', transition: 'all .3s', flexShrink: 0 }}><StepIcon id={step.id} done={done} /></div><div style={{ fontSize: isMobileViewport ? 11 : 12, fontWeight: active ? 800 : 600, color: active ? primary : (done ? text : '#94a3b8'), lineHeight: 1.2, maxWidth: '100%', overflowWrap: 'anywhere' }}>{step.label}</div>{active ? <div style={{ fontSize: 10, color: muted, marginTop: -2 }}>Updated</div> : null}</div>;
            })}
          </section>

          <section style={{ background: secondarySoft, borderRadius: 16, padding: isMobileViewport ? 16 : 20, display: 'flex', gap: 16, alignItems: 'center', flexWrap: isMobileViewport ? 'wrap' : 'nowrap', minWidth: 0 }}><div style={{ color: primaryDark, display: 'grid', placeItems: 'center', flexShrink: 0 }}>{isLocalQuote ? <FileText size={28} /> : (isLocalDropoff ? <Package size={28} /> : (isPickup ? <ShoppingBag size={28} /> : <Truck size={28} />))}</div><div style={{ minWidth: 0, flex: '1 1 220px' }}><div style={{ fontSize: 17, fontWeight: 800, color: primaryDark }}>{handoffTitle}</div><div style={{ marginTop: 3, fontSize: 14, color: '#1e516b', lineHeight: 1.45 }}>{handoffDescription}</div></div></section>
        ...
        <aside style={{ display: 'grid', gap: 20 }}>
          <section style={{ border: '1px solid #d9e4e8', borderRadius: 20, padding: isMobileViewport ? 18 : 24, background: '#fff', display: 'grid', gap: 18, boxShadow: '0 8px 24px rgba(15,23,42,.04)' }}>
            ...
            <div style={{ display: 'grid', gap: 5, minWidth: 0 }}><div style={{ color: muted, fontSize: 13 }}>Booking reference</div><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: isMobileViewport ? 'wrap' : 'nowrap', minWidth: 0 }}><strong style={{ color: text, fontSize: 17, fontFamily: displayFont, overflowWrap: 'anywhere', minWidth: 0 }}>{view.reference}</strong><button type="button" onClick={() => actions.copyTextToClipboard(view.reference, 'Booking reference copied.')} style={{ border: 0, background: 'transparent', color: primary, display: 'inline-flex', gap: 5, alignItems: 'center', fontWeight: 800, cursor: 'pointer', padding: 0, fontFamily: bodyFont }}> <Copy size={14} /> Copy</button></div></div>
            ...
            <div style={{ borderTop: '1px solid #eef2f7', paddingTop: 16, display: 'grid', gap: 12 }}><div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: isMobileViewport ? 'wrap' : 'nowrap', minWidth: 0 }}><div style={{ width: 58, height: 58, borderRadius: 13, overflow: 'hidden', background: soft, display: 'grid', placeItems: 'center', flexShrink: 0 }}><ServiceImage item={view.catalogItem} imageSources={view.imageSources} alt={view.serviceName} sizes="58px" width={58} height={58} fallbackLabel="" fallbackIcon={<PackageCheck size={22} color={primary} />} /></div><div style={{ minWidth: 0, flex: '1 1 160px' }}><div style={{ color: text, fontSize: 15, fontWeight: 900, overflowWrap: 'anywhere' }}>{view.serviceName || 'Service booking'}</div><div style={{ color: muted, fontSize: 13, marginTop: 4, overflowWrap: 'anywhere' }}>x {quantity}{view.serviceCategory ? ` - ${view.serviceCategory}` : ''}</div></div>{view.amount !== null ? <strong style={{ color: primaryDark, fontSize: 15, whiteSpace: 'nowrap', marginLeft: isMobileViewport ? 70 : 0 }}>{actions.money(view.amount)}</strong> : null}</div></div>
          ...
          <section style={{ border: `1px solid ${cardBorder}`, borderRadius: 18, padding: 18, background: soft, display: 'flex', gap: 12, alignItems: 'flex-start', minWidth: 0 }}><div style={{ width: 34, height: 34, borderRadius: 10, display: 'grid', placeItems: 'center', background: '#fff', color: primary, flexShrink: 0 }}><PartyPopper size={19} /></div><div style={{ minWidth: 0, overflowWrap: 'anywhere' }}><div style={{ fontWeight: 800, color: text }}>Thank you for your booking!</div><div style={{ marginTop: 4, color: muted, fontSize: 14 }}>We&apos;ll update you as your service progresses.</div></div></section>
        </aside>
      </div>
