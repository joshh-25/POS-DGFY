...
export function SimpleCatalogPagination({ currentPage, end, isMobileViewport, onPageChange, onPageSizeChange, pageSize, start, totalItems, totalPages }) {
  ...
  const buttonStyle = { minWidth: 40, minHeight: 40, borderRadius: 10, border: '1px solid #E4C98E', background: '#fff', color: '#0f172a', fontSize: 13, fontWeight: 600, cursor: 'pointer', padding: '0 10px' };
  const pageButtons = pages.map((page) => <button key={page} type="button" onClick={() => onPageChange(page)} style={{ ...buttonStyle, background: page === currentPage ? accent : '#fff', borderColor: page === currentPage ? accent : '#E4C98E', color: page === currentPage ? '#fff' : '#0f172a' }}>{page}</button>);
