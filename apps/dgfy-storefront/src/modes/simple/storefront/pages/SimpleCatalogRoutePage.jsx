...
export function SimpleCatalogRoutePage({ addToCart, catalogError, catalogItems, catalogSearch, catalogState, checkoutVoucherCode, setCheckoutVoucherCode, handleVoucherCardApply, handleVoucherCardRemove, filteredCatalog, filteredCatalogViewModel, getCartFlySourceRect, hasCatalogSearchQuery, isCategoryDropdownOpen, isMobileViewport, modeAdapter, onCategoryChange, onCategoryDropdownOpenChange, onPageChange, onPageSizeChange, onSearchChange, onSortChange, onViewDetails, onViewModeChange, page, pageEnd, pageSize, pageStart, refreshStorePageForTenantSetup, resolvedSection, sortOption, totalItems, totalPages, viewMode, categoryDropdownRef, mobileContentWidth }) {
  return (
    <section id="storefront-catalog-section" style={{ display: 'grid', gap: isMobileViewport ? 16 : 24, background: '#fff', padding: isMobileViewport ? '32px 0 24px' : '32px 0 52px', marginLeft: isMobileViewport ? undefined : 'calc(50% - 50vw)', width: isMobileViewport ? '100%' : '100vw', boxSizing: 'border-box' }}>
      <SimpleCatalogToolbar catalogSearch={catalogSearch} checkoutVoucherCode={checkoutVoucherCode} setCheckoutVoucherCode={setCheckoutVoucherCode} handleVoucherCardApply={handleVoucherCardApply} handleVoucherCardRemove={handleVoucherCardRemove} categoryDropdownRef={categoryDropdownRef} filteredCatalogViewModel={filteredCatalogViewModel} isCategoryDropdownOpen={isCategoryDropdownOpen} isMobileViewport={isMobileViewport} modeAdapter={modeAdapter} resolvedSection={resolvedSection} sortOption={sortOption} viewMode={viewMode} onCategoryChange={onCategoryChange} onCategoryDropdownOpenChange={onCategoryDropdownOpenChange} onSearchChange={onSearchChange} onSortChange={onSortChange} onViewModeChange={onViewModeChange} />
