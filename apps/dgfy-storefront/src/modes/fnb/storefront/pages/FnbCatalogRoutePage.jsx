...
export function FnbCatalogRoutePage({ addToCart, catalogError, catalogItems, catalogSearch, catalogState, checkoutVoucherCode, setCheckoutVoucherCode, handleVoucherCardApply, handleVoucherCardRemove, filteredCatalog, filteredCatalogViewModel, getCartFlySourceRect, hasCatalogSearchQuery, isCategoryDropdownOpen, isCompactPaginationViewport, isDesktopViewport, isMobileViewport, isTabletPaginationViewport, modeAdapter, mobileCatalogPadding, mobileContentWidth, onCategoryChange, onCategoryDropdownOpenChange, onPageChange, onPageSizeChange, onSearchChange, onSortChange, onViewDetails, onViewModeChange, page, pageEnd, pageSize, pageStart, refreshStorePageForTenantSetup, resolvedSection, sortOption, totalItems, totalPages, viewMode, categoryDropdownRef }) {
  const paginationAccent = '#f97316';
  const paginationAccentShadow = 'rgba(249,115,22,0.22)';

  return (
    <section id="storefront-catalog-section" style={{ display: 'grid', gap: isMobileViewport ? 16 : 24, marginTop: isMobileViewport ? 0 : 34, background: '#fff', padding: isMobileViewport ? '32px 0 24px' : '32px 0 52px', marginLeft: isMobileViewport ? undefined : 'calc(50% - 50vw)', width: isMobileViewport ? '100%' : '100vw', boxSizing: 'border-box' }}>
      <StorefrontCatalogToolbar FNB_CATEGORY_ICON_MAP={FNB_CATEGORY_ICON_MAP} STYLES={STYLES} catalogSearch={catalogSearch} checkoutVoucherCode={checkoutVoucherCode} setCheckoutVoucherCode={setCheckoutVoucherCode} handleVoucherCardApply={handleVoucherCardApply} handleVoucherCardRemove={handleVoucherCardRemove} filteredFnbViewModel={filteredCatalogViewModel} fnbCategoryDropdownRef={categoryDropdownRef} fnbMobileMenuInnerWidth={mobileContentWidth} fnbSortOption={sortOption} fnbViewMode={viewMode} isFnbCategoryDropdownOpen={isCategoryDropdownOpen} isMobileViewport={isMobileViewport} modeAdapter={modeAdapter} resolvedFnbSection={resolvedSection} setActiveServiceTab={onCategoryChange} setCatalogSearch={onSearchChange} setFnbSortOption={onSortChange} setFnbViewMode={onViewModeChange} setIsFnbCategoryDropdownOpen={onCategoryDropdownOpenChange} showViewToggle />
        ...
        <div style={{ maxWidth: 1320, margin: `${isMobileViewport ? 20 : 28}px auto 0`, width: '100%', boxSizing: 'border-box', padding: isMobileViewport ? mobileCatalogPadding : '0 24px' }}>
          ...
          {totalItems > 0 && totalPages > 1 && (
            <div style={{ marginTop: isMobileViewport ? 20 : 36, display: 'grid', gap: isCompactPaginationViewport ? 8 : 12, padding: isCompactPaginationViewport ? '12px 0' : isTabletPaginationViewport ? '14px 0' : '16px 0', paddingBottom: isCompactPaginationViewport ? 20 : 12, width: isMobileViewport ? mobileContentWidth : '100%', margin: isMobileViewport ? '0 auto' : 0, boxSizing: 'border-box' }}>
              <FnbCatalogPagination currentPage={page} end={pageEnd} isCompactPaginationViewport={isCompactPaginationViewport} isDesktopViewport={isDesktopViewport} onPageChange={onPageChange} onPageSizeChange={onPageSizeChange} pageSize={pageSize} pageStart={pageStart} totalItems={totalItems} totalPages={totalPages} paginationAccent={paginationAccent} paginationAccentShadow={paginationAccentShadow} />
            </div>
          )}
        </div>
      )}
...
function FnbCatalogPagination({ currentPage, end, isCompactPaginationViewport, isDesktopViewport, onPageChange, onPageSizeChange, pageSize, pageStart, totalItems, totalPages, paginationAccent, paginationAccentShadow }) {
  ...
  const buttonStyle = { minWidth: isCompactPaginationViewport ? 40 : 44, minHeight: isCompactPaginationViewport ? 40 : 44, borderRadius: 12, border: '1px solid #e5e7eb', background: '#fff', color: STYLES.colors.dark, fontWeight: 500, fontSize: 14, cursor: 'pointer', flexShrink: 0, padding: isCompactPaginationViewport ? '0 8px' : '0 10px' };
