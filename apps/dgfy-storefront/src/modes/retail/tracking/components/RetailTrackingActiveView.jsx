...
export function RetailTrackingActiveView({ actions, formatters, isMobileViewport, mapProps, viewModel }) {
  ...
  const statusIcon = isPickup
    ...
    : (activeStatus === 'confirmed' ? <Store size={24} strokeWidth={2.5} /> : activeStatus === 'preparing' ? <ChefHat size={24} strokeWidth={2.5} /> : activeStatus === 'packed' ? <Package size={24} strokeWidth={2.5} /> : activeStatus === 'out_for_delivery' ? <Bike size={24} strokeWidth={2.5} /> : activeStatus === 'completed' ? <Check size={24} strokeWidth={2.5} /> : <Clock3 size={24} strokeWidth={2.5} />);
