...
import { RetailTrackingActiveView } from './RetailTrackingActiveView.jsx';
import { RetailTrackingCompletedView } from './RetailTrackingCompletedView.jsx';

function RetailTrackingLoadingState({ selectedTrackingPin }) {
  return (
...
export function RetailTrackingRoutePage({
  actions,
  formatters,
  ...
  const viewModel = useMemo(() => {
    ...
    const reviewInviteByItemId = new globalThis.Map(
      reviewInvites
        .filter((invite) => String(invite?.target_type || '').trim() === 'Retail_item' && Number.isFinite(Number(invite?.target_id)))
    ...
    return {
      ...
      pickupBranchAddress,
      pickupBranchName,
      primaryReviewInvite: reviewInvites.find((invite) => String(invite?.target_type || '').trim() === 'Retail_item') || null,
  ...
  if (!viewModel.isMatchingTrackingPin) {
    return selectedTrackingPin && !trackingError ? <RetailTrackingLoadingState selectedTrackingPin={selectedTrackingPin} /> : null;
  }
  ...
  if (viewModel.activeStatus === 'completed' && showCompletedTrackingCard) {
    return <RetailTrackingCompletedView {...sharedViewProps} />;
  }
