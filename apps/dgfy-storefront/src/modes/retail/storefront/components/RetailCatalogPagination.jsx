...
export function RetailCatalogPagination({ currentPage, end, isMobileViewport, onPageChange, onPageSizeChange, pageSize, start, totalItems, totalPages }) {
  ...
  const buttonStyle = { minWidth: 36, minHeight: 36, borderRadius: 10, border: '1px solid #E2E8F0', background: '#fff', color: '#0f172a', fontSize: 13, fontWeight: 600, cursor: 'pointer', padding: '0 10px' };
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: isMobileViewport ? 'minmax(0, 1fr) auto' : 'minmax(0, 1fr) auto minmax(0, 1fr)', alignItems: 'center', columnGap: 16, rowGap: 10, marginTop: isMobileViewport ? 20 : 32 }}>
      ...
      <div className="no-scrollbar" style={{ display: 'flex', gridColumn: isMobileViewport ? '1 / -1' : '2', gridRow: isMobileViewport ? '2' : '1', justifyContent: isMobileViewport ? 'flex-start' : 'center', gap: 7, minWidth: 0, overflowX: 'auto', paddingBottom: 2 }}>
        ...
        {pages.map((page) => <button key={page} type="button" onClick={() => onPageChange(page)} style={{ ...buttonStyle, background: page === currentPage ? accent : '#fff', borderColor: page === currentPage ? accent : '#E2E8F0', color: page === currentPage ? '#fff' : '#0f172a', boxShadow: page === currentPage ? '0 8px 18px rgba(26,78,141,0.22)' : 'none' }}>{page}</button>)}
