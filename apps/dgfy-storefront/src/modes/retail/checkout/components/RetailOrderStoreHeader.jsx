...
export function RetailOrderStoreHeader({
  ...
  return (
    <section style={{ borderBottom: '1px solid #e2e8f0', background: '#fff', width: isResponsiveFlow ? '100%' : '100vw', marginLeft: isResponsiveFlow ? 0 : 'calc(50% - 50vw)', boxSizing: 'border-box', boxShadow: '0 6px 18px rgba(15,23,42,.05)' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto', width: '100%', padding: isResponsiveFlow ? '10px 16px' : isMobileViewport ? '12px 16px' : '18px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, boxSizing: 'border-box' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0, display: 'grid', gap: 2 }}>
            ...
            <div style={{ fontSize: isResponsiveFlow ? 18 : 20, fontWeight: 800, color: '#1e293b', lineHeight: 1.2, fontFamily: displayFont, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{storeName}</div>
          </div>
        </div>
