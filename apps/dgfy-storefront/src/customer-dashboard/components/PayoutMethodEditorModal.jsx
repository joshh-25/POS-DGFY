    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', alignItems: isMobileViewport ? 'flex-end' : 'center', justifyContent: 'center' }}>
      ...
      <div style={{ position: 'relative', background: THEME.surface, width: '100%', maxWidth: isMobileViewport ? '100%' : 480, borderRadius: isMobileViewport ? '24px 24px 0 0' : 24, padding: 24, boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04)', display: 'grid', gap: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.modalTitle, fontWeight: CUSTOMER_DASHBOARD_TYPOGRAPHY.modalTitleWeight, color: THEME.text }}>{isCreate ? 'Add Payout Method' : 'Edit Payout Method'}</h3>
          <button onClick={() => onClose()} style={{ background: THEME.bg, border: 'none', borderRadius: '50%', width: isMobileViewport ? 44 : 40, height: isMobileViewport ? 44 : 40, display: 'grid', placeItems: 'center', cursor: 'pointer', color: THEME.muted }}>
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={onSubmit} style={{ display: 'grid', gap: 16 }}>
          ...
          {!isCreate && (
            <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '12px 16px', background: THEME.bg, borderRadius: 12, marginTop: 4 }}>
              ...
              <div style={{ fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.body, fontWeight: 600, color: THEME.text }}>Set as default payout method</div>
            </label>
          )}

          {isCreate && (
            <div style={{ fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.secondary, background: THEME.infoBg, color: THEME.info, padding: '10px 14px', borderRadius: 10, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 8 }}>
            <button type="button" onClick={() => onClose()} style={{ minHeight: isMobileViewport ? 44 : 40, background: 'transparent', border: `1px solid ${THEME.border}`, borderRadius: 12, padding: '0 12px', fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.action, fontWeight: 700, color: THEME.text, cursor: 'pointer' }}>Cancel</button>
            <button type="submit" disabled={saving || !valid} style={{ minHeight: isMobileViewport ? 44 : 40, background: THEME.primary, border: 'none', borderRadius: 12, padding: '0 12px', fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.action, fontWeight: 700, color: '#fff', cursor: (saving || !valid) ? 'not-allowed' : 'pointer', opacity: (saving || !valid) ? 0.6 : 1 }}>
