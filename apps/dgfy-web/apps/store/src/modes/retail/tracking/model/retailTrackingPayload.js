...
export function unwrapRetailTrackingPayload(payload) {
...
export function toRetailTrackingViewState(trackingPayload) {
  const { raw, root, order, location } = unwrapRetailTrackingPayload(trackingPayload?.raw || {});
...
export function buildRetailTrackedOrderEntry({ fallbackPin = '', normalizedEntity, payload, fallbackStore, routeSlug, toSlug }) {
  const { raw, root, order, location } = unwrapRetailTrackingPayload(payload);
  const resolvedStore = fallbackStore || null;
  const storeSlug = toSlug(resolvedStore?.slug || routeSlug);
  if (!storeSlug) return null;
  const view = toRetailTrackingViewState({ raw: payload, normalized: normalizedEntity });
