...
import { buildRetailTrackedOrderEntry, toRetailTrackingViewState } from '../model/retailTrackingPayload.js';
...
export function useRetailTrackingRuntime({ checkoutTab, isRetailOrderSubpage, normalizeErrorMessage, requestJson, routeSlug, selectedStore, toSlug, trackingAdapterRegistry, trackingMode }) {
  ...
  const buildTrackedOrderEntryFromTrackingPayload = useCallback((payload, fallbackPin = '', normalizedEntity = null, fallbackStore = null) => (
    buildRetailTrackedOrderEntry({ fallbackPin, normalizedEntity, payload, fallbackStore: fallbackStore || selectedStore, routeSlug, toSlug })
  ...
  const handleTrack = useCallback(async () => {
    ...
    try {
      ...
      setTrackingResult(toRetailTrackingViewState(payload));
  ...
  // `checkoutTab === 'track' && isRetailOrderSubpage` is the same gate the
  ...
  const isTrackingViewActive = checkoutTab === 'track' && isRetailOrderSubpage;
  useEffect(() => {
  ...
  useEffect(() => {
    if (!(checkoutTab === 'track' && isRetailOrderSubpage) || !selectedStore?.slug) return undefined;
    ...
    const refreshSelected = async () => {
      ...
      try {
        ...
        const viewState = toRetailTrackingViewState(payload);
  ...
  }, [backgroundPinKey, checkoutTab, isRetailOrderSubpage, selectedPin, selectedStore?.slug]);
