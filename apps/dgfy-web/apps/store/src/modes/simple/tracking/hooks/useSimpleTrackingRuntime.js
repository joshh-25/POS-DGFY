...
import { buildSimpleTrackedOrderEntry, toSimpleTrackingViewState } from '../model/simpleTrackingPayload.js';
...
/**
 * Simple/MSME tracking runtime. It owns Simple's adapter selection, payload
 * normalization, local per-store order snapshots, and polling lifecycle.
 */
export function useSimpleTrackingRuntime({ checkoutTab, enabled = true, isSimpleOrderSubpage, normalizeErrorMessage, requestJson, routeSlug, selectedStore, toSlug, trackingAdapterRegistry }) {
  ...
  const fetchTrackingPayload = useCallback(async (pin) => {
    ...
    const adapterResult = await fetchNormalizedTrackingEntity({
      ...
      input: { mode: 'simple', storeSlug: selectedStore.slug, rawReference: normalizedPin },
      requestJson
    });
  ...
  }, [requestJson, selectedStore?.slug, trackingAdapterRegistry]);

  const buildTrackedOrderEntryFromTrackingPayload = useCallback((payload, fallbackPin = '', normalizedEntity = null, fallbackStore = null) => (
    buildSimpleTrackedOrderEntry({
      fallbackPin,
      normalizedEntity,
      payload,
      fallbackStore: fallbackStore || selectedStore,
      routeSlug,
      toSlug
    })
  ...
  const handleTrack = useCallback(async () => {
    ...
    try {
      ...
      setTrackingResult(toSimpleTrackingViewState(payload));
  ...
  useEffect(() => {
    ...
    if (!completed) { setShowCompletedTrackingCard(false); return undefined; }
  ...
  }, [enabled, routeSlug, selectedStore?.slug, selectedTrackingPin, setGuestTrackedOrdersIfChanged, toSlug, trackingPinInput]);
  ...
  const isTrackingViewActive = enabled && checkoutTab === 'track' && isSimpleOrderSubpage;
  useEffect(() => {
  ...
  useEffect(() => {
    if (!enabled || !(checkoutTab === 'track' && isSimpleOrderSubpage) || !selectedStore?.slug) return undefined;
    ...
    const refreshSelected = async () => {
      ...
      try {
        ...
        const viewState = toSimpleTrackingViewState(payload);
  ...
  }, [backgroundPinKey, checkoutTab, enabled, isSimpleOrderSubpage, selectedPin, selectedStore?.slug]);
  ...
  return {
    ...
    selectedTrackingPin,
    setGuestTrackedOrders: setGuestTrackedOrdersIfChanged,
