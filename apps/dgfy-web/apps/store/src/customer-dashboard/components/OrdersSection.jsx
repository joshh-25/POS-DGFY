...
export function OrdersSection({
    ...
    <div key={`all-order-${order.reference}`} style={{ background: theme.surface, borderRadius: 16, border: `1px solid ${theme.border}`, padding: isMobileViewport ? 16 : 24, display: isMobileViewport ? 'flex' : 'grid', gridTemplateColumns: isMobileViewport ? undefined : 'minmax(0, 1fr) 180px 360px', justifyContent: 'space-between', alignItems: isMobileViewport ? 'stretch' : 'center', flexWrap: isMobileViewport ? 'wrap' : 'nowrap', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, minWidth: 0, flex: '1 1 280px' }}>
        ...
        <div style={{ minWidth: 0 }}>
          <button type="button" onClick={() => onOpenStorefront(order)} style={{ fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.cardTitle, fontWeight: 700, color: theme.text, marginBottom: 4, background: 'transparent', border: 'none', padding: 0, cursor: order.store_slug ? 'pointer' : 'default', textAlign: 'left' }}>{order.store_name || 'DGFY Store'}</button>
          <div style={{ fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.secondary, color: theme.muted }}>#{order.reference}</div>
        </div>
      </div>
      <div style={{ textAlign: isMobileViewport ? 'left' : 'right', minWidth: isMobileViewport ? '100%' : 160, justifySelf: isMobileViewport ? undefined : 'end' }}>
        <div style={{ fontSize: isMobileViewport ? CUSTOMER_DASHBOARD_TYPOGRAPHY.metric.mobile : CUSTOMER_DASHBOARD_TYPOGRAPHY.metric.desktop, fontWeight: 800, color: theme.text, marginBottom: 8 }}>{money(order.total_amount)}</div>
        <StatusBadge status={order.status_label || order.status} />
        <div style={{ fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.caption, color: theme.muted, marginTop: 8 }}>{formatDate(order.occurred_at)}</div>
      </div>
      <div style={{ display: 'flex', gap: 12, flexWrap: isMobileViewport ? 'wrap' : 'nowrap', width: isMobileViewport ? '100%' : 360, justifySelf: isMobileViewport ? undefined : 'end', alignItems: 'center', justifyContent: isMobileViewport ? 'flex-start' : 'flex-end' }}>
        <button type="button" onClick={() => onTrackReference(order)} disabled={customerTrackLoadingReference === order.reference} style={{ background: theme.surface, border: `1px solid ${theme.border}`, color: theme.primary, borderRadius: 8, minHeight: isMobileViewport ? CUSTOMER_DASHBOARD_TYPOGRAPHY.controlHeight.mobile : CUSTOMER_DASHBOARD_TYPOGRAPHY.controlHeight.desktop, padding: '0 16px', fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.compactAction, fontWeight: 600, cursor: customerTrackLoadingReference === order.reference ? 'wait' : 'pointer' }}>{customerTrackLoadingReference === order.reference ? 'Loading...' : 'Track Order'}</button>
        <button type="button" onClick={() => onTrackReference(order)} style={{ background: theme.primary, border: 'none', color: '#FFF', borderRadius: 8, minHeight: isMobileViewport ? CUSTOMER_DASHBOARD_TYPOGRAPHY.controlHeight.mobile : CUSTOMER_DASHBOARD_TYPOGRAPHY.controlHeight.desktop, padding: '0 16px', fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.compactAction, fontWeight: 600, cursor: 'pointer' }}>View Order</button>
        {allowReview && order?.allowed_actions?.review === true && Array.isArray(order?.review_targets) && order.review_targets.length > 0 ? (
          <button type="button" onClick={() => onOpenReview(order)} style={{ background: 'transparent', border: `1px solid ${theme.border}`, color: theme.primary, borderRadius: 8, padding: '8px 16px', fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.compactAction, fontWeight: 600, cursor: 'pointer' }}>Review Order</button>
  ...
  return (
    <div style={{ display: 'grid', gap: 24 }}>
      <div><h2 style={{ fontSize: isMobileViewport ? CUSTOMER_DASHBOARD_TYPOGRAPHY.pageTitle.mobile : CUSTOMER_DASHBOARD_TYPOGRAPHY.pageTitle.desktop, fontWeight: CUSTOMER_DASHBOARD_TYPOGRAPHY.pageTitleWeight, color: theme.text, margin: 0 }}>Orders</h2><p style={{ margin: '8px 0 0', fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.pageSubtitle, color: theme.muted }}>Track your in-progress orders, review completed purchases, and revisit your feedback.</p></div>
      <div data-testid="customer-orders-tabs" style={{ display: 'flex', flexWrap: 'nowrap', gap: isMobileViewport ? 8 : 10, padding: isMobileViewport ? '0 0 4px' : 0, borderBottom: `1px solid ${theme.border}`, overflowX: 'auto', overscrollBehaviorX: 'contain', scrollbarWidth: 'thin' }}>
        {tabs.map((tab) => {
      ...
      <div style={{ display: 'grid', gap: 16 }}>
        {activeTab === 'reviews' ? reviews.length === 0 ? <EmptyState title="No submitted reviews yet" desc="Completed orders you review will appear here once review history is available in your account feed." /> : reviews.map((review, index) => (
          <div key={`review-history-${review.review_id || index}`} style={{ background: theme.surface, borderRadius: 16, border: `1px solid ${theme.border}`, padding: isMobileViewport ? 16 : 20, display: 'grid', gap: 10 }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}><div><div style={{ fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.cardTitle, fontWeight: 700, color: theme.text }}>{review.store_name || review.title || 'Reviewed order'}</div><div style={{ fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.secondary, color: theme.muted, marginTop: 4 }}>{prettyStatus(review.target_type || 'Review')}</div></div><StatusBadge status={review.status_label || review.status || 'Reviewed'} /></div>{review.comment ? <div style={{ fontSize: CUSTOMER_DASHBOARD_TYPOGRAPHY.body, color: theme.text, lineHeight: 1.5 }}>{review.comment}</div> : null}</div>
