...
export function CustomerDashboardShell({ presentation, isMobileViewport, activeNav, setActiveNav, notifications, unreadNotificationCount, accountIdentityInitials, accountIdentityName, onClose, onSignOut, onHelp, onMarkNotificationRead, onMarkAllNotificationsRead, onTrackReference, children }) {
  ...
  const headerStyle = isDrawer
    ? { height: 72, background: theme.surface, borderBottom: `1px solid ${theme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobileViewport ? '0 16px' : '0 24px', position: 'sticky', top: 0, zIndex: 30, flexShrink: 0, overflow: 'visible' }
    : { height: 72, background: theme.surface, borderBottom: `1px solid ${theme.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', position: 'fixed', top: 0, left: isMobileViewport ? 0 : 260, right: 0, zIndex: 30, flexShrink: 0, overflow: 'visible' };
