...
/**
 * The in-store host for `frontend/Pages/DgfyResetPasswordPage.jsx`. Moved, not
 * redesigned: same markup, same copy, same two-step flow. Only the sign-in link
 * target changes, from skupervisor's /dgfy/auth to the in-app /login route.
 */
export default function StorefrontResetPasswordPage() {
  ...
  const buildSignInPath = (email) => `${SIGN_IN_ROUTE}${buildDgfyRouteSearch({
  ...
  const handleCompletePasswordReset = async (event) => {
    ...
    try {
      ...
      navigate(buildSignInPath(resetForm.email), {
        replace: true,
        ...
        <div className="mx-auto w-full max-w-[480px] px-6 py-8 sm:px-8 sm:py-12">
          ...
          <div className="flex flex-col gap-6">
            ...
            <p className="text-center text-sm" style={{ color: '#64748B' }}>
              <Link
                to={buildSignInPath(resetForm.email)}
