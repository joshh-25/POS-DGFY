    ...
    const renderUploadStep = () => (
        <div className="space-y-6">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm">
                <p className="font-medium text-amber-800 mb-1">⚠️ Limitation Note</p>
                <p className="text-amber-700">
                    This import is for <strong>Supplier Details only</strong> (Name, Contact, Address).
                    Assigning Items to Suppliers must be done via the main interface after import.
                </p>
            </div>
        </div>
    );
    ...
    const renderResultStep = () => (
        <div className="space-y-6 text-center py-4">
            ...
            <div>
                ...
                <p className="text-slate-600">Your suppliers have been successfully imported.</p>
            </div>
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-left max-w-md mx-auto">
                    ...
                    <div className="space-y-1 text-sm text-red-700 max-h-32 overflow-y-auto">
                        {importResult.results?.failed?.map((f, i) => (
                            <p key={i}>Row {f.rowNumber} ({f.name}): {f.errors?.join(', ')}</p>
                        ))}
                    </div>
                </div>
            )}
                        ...
                        <>
                            ...
                            >
                                {loading ? (
                                    ...
                                    <><CheckCircle2 className="w-4 h-4 mr-2" /> Confirm Import ({previewData?.validRows} rows)</>
                                )}
                            </Button>
                        </>
                    )}
