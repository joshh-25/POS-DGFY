...
const checkoutPayloadSource = () => fs.readFileSync(path.join(appRoot, 'checkout/buildFnbCheckoutPayload.js'), 'utf8');
...
describe('Food & Beverage storefront contract', () => {
  ...
  it('wires promo-code checkout through backend-validated quote and checkout payloads', () => {
    const source = appSource();
    ...
    expect(promoSource).toContain('statusMessage');
    expect(promoSource).toContain('Discount');
    expect(summarySource).toContain('promoSlot');
    expect(checkoutPayloadSource()).toContain('promo_code:');
    expect(source).toContain('Please click Quote first before checkout.');
    expect(source).toContain('Promo code applied successfully.');
  });
