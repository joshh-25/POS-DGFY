/**
 * Default/Retail order page's read-only item list. Mirrors
 ...
 */
export function DefaultOrderReviewItemsList({
  cart = [],
  ...
  return (
    <div style={{ maxHeight: isMobileViewport ? 260 : 320, overflowY: 'auto', display: 'grid', gap: 8 }}>
      {cart.map((line) => {
        ...
        return (
          <div key={`default-order-summary-${line.item_id}`} style={{ border: '1px solid #e2e8f0', borderRadius: 12, background: '#fff', padding: '9px 10px', display: 'flex', justifyContent: 'space-between', gap: 8, fontSize: 12 }}>
