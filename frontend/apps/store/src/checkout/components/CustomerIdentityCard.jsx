...
export function CustomerIdentityCard({
  ...
  return (
    <section style={{ border: '1px solid #dbeafe', borderRadius: 16, background: 'linear-gradient(135deg,#eff6ff,#ffffff)', padding: isMobileViewport ? 14 : 16, display: 'grid', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: isMobileViewport ? 'flex-start' : 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: isMobileViewport ? 'flex-start' : 'center', gap: 12, minWidth: 0, flex: 1 }}>
          ...
          <div style={{ display: 'grid', gap: 4, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 12, color: '#64748b', fontFamily: bodyFont }}>{subtitle}</div>
          </div>
        </div>
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: isMobileViewport ? 12 : 18, paddingTop: 2 }}>
