...
import POSCheckoutTerminal from '../../../../src/features/pos/components/POSCheckoutTerminal.jsx';
import TerminalLockDrawer from './TerminalLockDrawer.jsx';
import TerminalWorkspaceSidebar from './TerminalWorkspaceSidebar.jsx';
import TerminalOperationsWorkspace from '../../../../src/features/pos/components/TerminalOperationsWorkspace.jsx';
...
export default function TerminalPageLayout({
  ...
  const onlineIndicatorClassName = isOnline ? 'dgfy-pos-dot dgfy-pos-dot--online' : 'dgfy-pos-dot dgfy-pos-dot--offline';
  ...
  const compactBellClassName = IS_DGFY_POS_SURFACE
    ? 'relative grid h-8 w-8 shrink-0 place-items-center rounded-xl text-[#c2410c] hover:bg-orange-50 lg:hidden'
    : 'hidden';
  const compactIdentityClassName = IS_DGFY_POS_SURFACE
    ? 'hidden'
    : 'hidden';
  const desktopBellClassName = IS_DGFY_POS_SURFACE
    ? 'relative hidden h-10 w-10 shrink-0 place-items-center rounded-xl text-[#c2410c] hover:bg-orange-50 lg:grid'
    : 'relative grid h-10 w-10 shrink-0 place-items-center rounded-xl text-[#c2410c] hover:bg-orange-50';
  ...
  const notificationPanel = notificationsOpen ? createPortal(
    <div className="fixed inset-0 z-[120]" onClick={() => setNotificationsOpen(false)}>
      ...
      >
        <div className="absolute right-4 top-0 z-10 h-5 w-5 -translate-y-[62%] rotate-45 border-l border-t border-amber-200 bg-[#c2410c]" />
        <div className="overflow-hidden rounded-3xl border border-amber-100 bg-white shadow-2xl shadow-slate-950/15">
          <div className="bg-gradient-to-r from-[#c2410c] to-[#ea580c] px-4 py-3 text-white">
            <p className="text-lg font-black">Notifications</p>
            <p className="mt-0.5 text-xs text-orange-100">Review alerts without leaving the current screen.</p>
          </div>
          <div className="max-h-[22rem] overflow-y-auto bg-[#fffaf5]">
            {notifications.length > 0 ? notifications.map((item) => (
              ...
              >
                <span className="mt-2 h-3 w-3 shrink-0 rounded-full bg-[#ea580c]" />
                <div className="min-w-0 flex-1">
                  ...
                  {item.actionLabel ? (
                    <span className="mt-2 inline-flex text-[11px] font-bold uppercase tracking-wide text-[#c2410c]">
                      {item.actionLabel}
                    </span>
                  ) : null}
                </div>
          ...
          {notifications.length > 0 && (
            <button
              ...
              className="block w-full bg-gradient-to-r from-[#c2410c] to-[#ea580c] px-4 py-3 text-center text-sm font-black text-white transition hover:from-[#b45309] hover:to-[#c2410c]"
            >
        ...
        <div className={`dgfy-pos-shell h-[100dvh] min-h-screen min-h-[100dvh] overflow-hidden ${shellLayoutClassName}`}>
                ...
                <div className={headerShellClassName}>
                    <div className="flex min-w-0 items-center justify-between gap-3">
                        ...
                        <button
                            ...
                            disabled={locked}
                            className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl text-[#c2410c] ${locked ? 'cursor-not-allowed opacity-45' : 'hover:bg-orange-50'}`}
                        ...
                        <div className="min-w-0 flex-1">
                            <div className="flex min-w-0 flex-wrap items-center gap-2">
                                <span className="rounded-full bg-orange-100 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-[#c2410c]">
                                    DGFY POS
                                </span>
                                <span className="dgfy-pos-terminal-badge rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.24em]">
                                    {terminalIdentityDisplay}
                                </span>
                                <span className="inline-flex items-center gap-2 rounded-full border border-emerald-100 bg-white px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-[#475569]">
                                    <span className={onlineIndicatorClassName} />
                                    {isOnline ? 'Online' : 'Offline'}
                                </span>
                            </div>
                            <h1 className="mt-2 min-w-0 truncate text-[18px] font-black tracking-tight text-[#0F172A] lg:text-[22px]">
                                {activeHeaderTitle}
                            </h1>
                    ...
                    <div className={compactIdentityClassName}>
                        <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-gradient-to-br from-amber-400 to-orange-600 text-white shadow-lg shadow-orange-950/20">
            ...
            {!isDesktopWide && mobileNavOpen && (
                <div className={overlayContainerClassName}>
                    ...
                    <div className="dgfy-pos-scrollbar-hidden absolute left-0 top-0 h-full w-[82%] max-w-[304px] overflow-y-auto p-3 shadow-2xl shadow-slate-950/30" style={{ background: 'var(--pos-shell-sidebar)' }}>
                        <div className="mb-2 flex min-w-0 items-center justify-between gap-3">
                            ...
                            <button
                                ...
                                className="grid h-10 w-10 shrink-0 place-items-center rounded-xl text-amber-200 hover:bg-white/10"
