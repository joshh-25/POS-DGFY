...
describe('storefront business hours helpers', () => {
  ...
  it('applies a selected range only to selected days', () => {
    ...
    expect(hours.weekly.mon).toEqual({ enabled: true, open: '09:00', close: '17:00' });
    expect(hours.weekly.wed).toEqual({ enabled: true, open: '09:00', close: '17:00' });
    expect(hours.weekly.tue).toEqual({ enabled: true, open: '09:00', close: '18:00' });
  });
  ...
  it('falls back invalid apply times to safe defaults', () => {
    ...
    expect(hours.weekly.fri).toEqual({ enabled: true, open: '09:00', close: '17:00' });
  });
  ...
  it('normalizes legacy display strings into the weekly schema', () => {
    ...
    expect(normalized.weekly.mon).toEqual({ enabled: true, open: '09:00', close: '17:00' });
    expect(normalized.weekly.tue).toEqual({ enabled: true, open: '09:00', close: '17:00' });
