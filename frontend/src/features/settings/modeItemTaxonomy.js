import { getUomGroup, normalizeUom } from '../../utils/uomConverter.js';
import { normalizeWorkflowMode } from './workflowMode.js';
...
const buildFinancialProfile = ({
  stock_behavior,
  cost_visibility = stock_behavior === STOCK.EXEMPT
...
const preset = ({
  ...
  allowed_uom_groups = [],
  allowed_uoms = [],
  stock_behavior = STOCK.BEARING,
  fifo_enabled = stock_behavior === STOCK.BEARING,
  max_capacity = 100,
  financial_profile = {}
}) => Object.freeze({
  key,
...
export const MODE_ITEM_TAXONOMY = Object.freeze({
  ...
  services: taxonomy({
    ...
    presets: [
      preset({ key: 'service', label: 'Service', category: 'service', default_unit: 'service', allowed_uom_groups: ['presentation', 'time'], allowed_uoms: ['service', 'session', 'booking', 'hour'], stock_behavior: STOCK.EXEMPT, fifo_enabled: false, max_capacity: 1, financial_profile: { sale_price_visibility: ITEM_FINANCIAL_VISIBILITY.VISIBLE } }),
  ...
  hospitality: taxonomy({
    ...
    presets: [
      preset({ key: 'room_night', label: 'Room Night / Accommodation', category: 'service', default_unit: 'room_night', allowed_uom_groups: ['time', 'presentation'], allowed_uoms: ['room_night', 'booking'], stock_behavior: STOCK.EXEMPT, fifo_enabled: false, max_capacity: 1, financial_profile: { sale_price_visibility: ITEM_FINANCIAL_VISIBILITY.VISIBLE } }),
      preset({ key: 'paid_amenity', label: 'Paid Amenity / Add-on', category: 'service', default_unit: 'service', allowed_uom_groups: ['presentation', 'time'], allowed_uoms: ['service', 'booking', 'hour'], stock_behavior: STOCK.EXEMPT, fifo_enabled: false, max_capacity: 1, financial_profile: { sale_price_visibility: ITEM_FINANCIAL_VISIBILITY.VISIBLE } }),
      preset({ key: 'facility_booking', label: 'Facility Booking', category: 'service', default_unit: 'booking', allowed_uom_groups: ['presentation', 'time'], allowed_uoms: ['booking', 'hour'], stock_behavior: STOCK.EXEMPT, fifo_enabled: false, max_capacity: 1, financial_profile: { sale_price_visibility: ITEM_FINANCIAL_VISIBILITY.VISIBLE } }),
