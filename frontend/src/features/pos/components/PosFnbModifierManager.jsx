...
export default function PosFnbModifierManager({ groups = [], items = [], locations = [], busy = false, canManage = true, onCreate, onUpdate }) {
  ...
  useEffect(() => {
    if (!editingId) return;
    const current = groups.find((group) => Number(group.modifier_group_id) === Number(editingId));
    if (current) setDraft(normalizeGroup(current));
  }, [editingId, groups]);
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
      <section className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {groups.map((group) => (
            <button type="button" key={group.modifier_group_id} disabled={!canManage} onClick={() => setEditingId(group.modifier_group_id)} className={`rounded-lg border p-3 text-left disabled:cursor-default ${Number(editingId) === Number(group.modifier_group_id) ? 'border-teal-500 ring-2 ring-teal-100' : 'border-slate-200'}`}>
