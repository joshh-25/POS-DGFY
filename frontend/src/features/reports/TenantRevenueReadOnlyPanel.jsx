    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4"><h3 className="font-semibold">Settlement statements</h3><div className="mt-3 space-y-2">{settlements.map((batch) => <div key={batch.settlement_batch_id} className="rounded-lg bg-slate-50 p-3 text-sm"><div className="font-mono text-xs">{batch.batch_number}</div><div className="mt-1 flex justify-between"><span className="capitalize">{String(batch.status).replace(/_/g, ' ')}</span><strong className="tabular-nums">{php(batch.payout_centavos)}</strong></div><div className="mt-1 text-xs text-slate-500">Transfer: {batch.actual_payout_at ? new Date(batch.actual_payout_at).toLocaleDateString('en-PH') : 'Not paid'}</div></div>)}{!settlements.length ? <p className="text-sm text-slate-500">No settlement statements yet.</p> : null}</div></div>
        </div>
      </div>
    </div>
  );
